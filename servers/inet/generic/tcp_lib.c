...
void tcp_extract_ipopt(tcp_conn_t *tcp_conn,mnx_ip_hdr_t *ip_hdr)
{
	int ip_hdr_len;
...
void tcp_extract_tcpopt(tcp_conn_t *tcp_conn,tcp_hdr_t *tcp_hdr,mnx_size_t *mssp)
{
...
acc_t *tcp_make_header(tcp_conn_t *tcp_conn, mnx_ip_hdr_t **ref_ip_hdr,
				tcp_hdr_t **ref_tcp_hdr, acc_t *data)
{
	...
	if (tcp_conn->tc_remipopt || tcp_conn->tc_tcpopt){
		...
		ip_hdr= (mnx_ip_hdr_t *)ptr2acc_data(hdr_acc);
