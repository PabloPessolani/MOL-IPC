...
typedef struct tcp_port
{
	...
	int tp_ipfd;
	acc_t *tp_pack;
	mnx_ipaddr_t tp_ipaddr;
	mnx_ipaddr_t tp_subnetmask;
	u16_t tp_mtu;
...
typedef struct tcp_fd
{
	...
	mnx_size_t tf_write_offset;
	mnx_size_t tf_write_count;
	mnx_size_t tf_read_offset;
	mnx_size_t tf_read_count;
...
typedef struct tcp_conn
{
	...
	mnx_ipaddr_t tc_locaddr;
	tcpport_t tc_remport;
	mnx_ipaddr_t tc_remaddr;
	...
	struct inet_timer tc_transmit_timer;
...
/* tcp_recv.c */
void tcp_frag2conn ( tcp_conn_t *tcp_conn, mnx_ip_hdr_t *ip_hdr,
	tcp_hdr_t *tcp_hdr, acc_t *tcp_data, mnx_size_t data_len );
...
void tcp_release_retrans ( tcp_conn_t *tcp_conn, u32_t seg_ack,
	u16_t new_win );
...
void tcp_close_connection ( tcp_conn_t *tcp_conn,
	int error );
void tcp_port_write ( tcp_port_t *tcp_port );
void tcp_shutdown ( tcp_conn_t *tcp_conn );

/* tcp_lib.c */
void tcp_extract_ipopt ( tcp_conn_t *tcp_conn,
	mnx_ip_hdr_t *ip_hdr );
void tcp_extract_tcpopt ( tcp_conn_t *tcp_conn,
	tcp_hdr_t *tcp_hdr, mnx_size_t *mssp );
void tcp_get_ipopt ( tcp_conn_t *tcp_conn, ip_hdropt_t
	*ip_hdropt );
void tcp_get_tcpopt ( tcp_conn_t *tcp_conn, tcp_hdropt_t
	*tcp_hdropt );
acc_t *tcp_make_header ( tcp_conn_t *tcp_conn,
	mnx_ip_hdr_t **ref_ip_hdr, tcp_hdr_t **ref_tcp_hdr, acc_t *data );
u16_t tcp_pack_oneCsum ( mnx_ip_hdr_t *ip_hdr, acc_t *tcp_pack );
int tcp_check_conn ( tcp_conn_t *tcp_conn );
void tcp_print_pack ( mnx_ip_hdr_t *ip_hdr, tcp_hdr_t *tcp_hdr );
...
int tcp_su4listen ( tcp_fd_t *tcp_fd, tcp_conn_t *tcp_conn,
	int do_listenq );
