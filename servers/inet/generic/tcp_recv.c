...
static void create_RST( tcp_conn_t *tcp_conn,
	mnx_ip_hdr_t *ip_hdr, tcp_hdr_t *tcp_hdr, int data_len );
static void process_data( tcp_conn_t *tcp_conn,
	tcp_hdr_t *tcp_hdr, acc_t *tcp_data, int data_len );
static void process_advanced_data( tcp_conn_t *tcp_conn,
	tcp_hdr_t *tcp_hdr, acc_t *tcp_data, int data_len );

void tcp_frag2conn(tcp_conn_t *tcp_conn, mnx_ip_hdr_t *ip_hdr, tcp_hdr_t *tcp_hdr,
			acc_t *tcp_data, mnx_size_t data_len)
{
	tcp_fd_t *connuser;
	int tcp_hdr_flags;
...
static  void process_data(tcp_conn_t *tcp_conn, tcp_hdr_t *tcp_hdr,
...
static  void create_RST(tcp_conn_t *tcp_conn, mnx_ip_hdr_t *ip_hdr, 
				tcp_hdr_t *tcp_hdr, int data_len)
{
	acc_t *tmp_ipopt, *tmp_tcpopt, *tcp_pack;
	acc_t *RST_acc;
	mnx_ip_hdr_t *RST_ip_hdr;
	tcp_hdr_t *RST_tcp_hdr;
	mnx_size_t pack_size, ip_hdr_len, mss;
...
void tcp_fd_read(tcp_conn_t *tcp_conn, int enq)	/* Enqueue writes. */
{
	tcp_fd_t *tcp_fd;
	mnx_size_t data_size, read_size;
	acc_t *data;
	...
	if (read_size){
		if (urg && !(tcp_fd->tf_flags & TFF_RECV_URG)){
			...
			if (tcp_fd->tf_read_offset)	{
				...
				tcp_reply_read(tcp_fd, EMOLNOURG);
			}
			return;
		}
...
void
tcp_rsel_read(tcp_conn_t *tcp_conn)
{
	tcp_fd_t *tcp_fd;
