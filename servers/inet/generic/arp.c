...
{
	mnx_ethaddr_t a46_dstaddr;
	mnx_ethaddr_t a46_srcaddr;
...
typedef struct arp_port
{
	...
	mnx_ethaddr_t ap_ethaddr;	/* Ethernet address of this port */
	mnx_ipaddr_t ap_ipaddr;		/* IP address of this port */

	struct arp_req
	{
		inet_timer_t ar_timer;
		int ar_entry;
		int ar_req_count;
	} ap_req[AP_REQ_NR];
...
typedef struct arp_cache
{
	int ac_flags;
	int ac_state;
	mnx_ethaddr_t ac_ethaddr;
	mnx_ipaddr_t ac_ipaddr;
	arp_port_t *ac_port;
	time_t ac_expire;
	time_t ac_lastuse;
} arp_cache_t;
...
arp_port_t *arp_port_table;
arp_cache_t *arp_cache;
int arp_cache_nr;

acc_t *arp_getdata ( int fd, mnx_size_t offset,
	mnx_size_t count, int for_ioctl );
int arp_putdata ( int fd, mnx_size_t offset,
	acc_t *data, int for_ioctl );
void arp_main ( arp_port_t *arp_port );
void arp_timeout ( int ref, inet_timer_t *timer );
void setup_write ( arp_port_t *arp_port );
static void setup_read ( arp_port_t *arp_port );
void do_reclist ( event_t *ev, ev_arg_t ev_arg );
void process_arp_pkt ( arp_port_t *arp_port, acc_t *data );
void client_reply ( arp_port_t *arp_port,
	mnx_ipaddr_t ipaddr, mnx_ethaddr_t *ethaddr );
arp_cache_t *find_cache_ent ( arp_port_t *arp_port,
	mnx_ipaddr_t ipaddr );
arp_cache_t *alloc_cache_ent ( int flags );
void arp_buffree ( int priority );
#ifdef BUF_CONSISTENCY_CHECK
void arp_bufcheck ( void );
#endif
...
void arp_prep( void )
{
	...
	arp_port_table= inet_alloc(eth_conf_nr * sizeof(arp_port_table[0]));
	...
	arp_cache= inet_alloc(arp_cache_nr * sizeof(arp_cache[0]));
}
...
acc_t *arp_getdata (int fd, mnx_size_t offset,mnx_size_t count,int for_ioctl)
{
	arp_port_t *arp_port;
	acc_t *data;
	int result;
...
int arp_putdata (int fd, mnx_size_t offset, acc_t *data, int for_ioctl)
{
	arp_port_t *arp_port;
	int result;
...
void process_arp_pkt(arp_port_t *arp_port, acc_t *data)
{
	...
	struct arp_req *reqp;
	time_t curr_time;
	mnx_ipaddr_t spa, tpa;

	curr_time= get_time();

	arp= (arp46_t *)ptr2acc_data(data);
	memcpy(&spa, arp->a46_spa, sizeof(mnx_ipaddr_t));
	memcpy(&tpa, arp->a46_tpa, sizeof(mnx_ipaddr_t));
	...
	if (do_reply) {
		...
		memcpy (arp->a46_spa, &tpa, sizeof(mnx_ipaddr_t));
		arp->a46_tha= ce->ac_ethaddr;
		memcpy (arp->a46_tpa, &ce->ac_ipaddr, sizeof(mnx_ipaddr_t));
...
void client_reply (arp_port_t *arp_port, mnx_ipaddr_t ipaddr, mnx_ethaddr_t *ethaddr)
{
	(*arp_port->ap_arp_func)(arp_port->ap_ip_port, ipaddr, ethaddr);
}

arp_cache_t *find_cache_ent(arp_port_t *arp_port, mnx_ipaddr_t ipaddr)
{
	arp_cache_t *ce;
	int i;
	unsigned hash;
...
void arp_timeout (int ref, inet_timer_t *timer)
{
	...
	arp->a46_sha= arp_port->ap_ethaddr;
	memcpy (arp->a46_spa, &arp_port->ap_ipaddr, sizeof(mnx_ipaddr_t));
	memset(&arp->a46_tha, '\0', sizeof(mnx_ethaddr_t));
	memcpy (arp->a46_tpa, &ce->ac_ipaddr, sizeof(mnx_ipaddr_t));
