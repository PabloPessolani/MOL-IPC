...
psip_port_t *psip_port_table;
psip_fd_t psip_fd_table[PSIP_FD_NR];

int psip_open( int port, int srfd,
	get_userdata_t get_userdata, put_userdata_t put_userdata,
	put_pkt_t pkt_pkt, select_res_t select_res );
int psip_ioctl( int fd, ioreq_t req );
int psip_read( int fd, mnx_size_t count );
int psip_write( int fd, mnx_size_t count );
int psip_select( int port_nr, unsigned operations );
void psip_close( int fd );
int psip_cancel( int fd, int which_operation );
void promisc_restart_read( psip_port_t *psip_port );
int psip_setopt( psip_fd_t *psip_fd, nwio_psipopt_t *newoptp );
void psip_buffree( int priority );
void check_promisc( psip_port_t *psip_port );
#ifdef BUF_CONSISTENCY_CHECK
void psip_bufcheck( void );
#endif
static void reply_thr_put( psip_fd_t *psip_fd, int reply,
	int for_ioctl );
static void reply_thr_get( psip_fd_t *psip_fd, int reply,
	int for_ioctl );

void psip_prep(void )
{
	SVRDEBUG("\n");
	psip_port_table= inet_alloc(psip_conf_nr * sizeof(psip_port_table[0]));
}
...
int psip_enable(int port_nr, int ip_port_nr)
{
	...
	sr_add_minor(if2minor(psip_conf[port_nr].pc_ifno, PSIP_DEV_MINOR),
	...
	for(;;) {
		...
		if (buf_size+extrasize <= psip_fd->pf_rd_count) {
			...
			if (extrasize) {
				...
				*(mnx_ipaddr_t *)(ptr2acc_data(acc))= dest;
				acc->acc_next= pack;
				pack= acc; acc= NULL;
				buf_size += extrasize;
			}
		...
		result1= (*psip_fd->pf_put_userdata)(psip_fd->pf_srfd,
				(mnx_size_t)result, NULL, FALSE);
		assert(result1 == NW_OK);
		if (result == EMOLPACKSIZE)
			continue;
		return NW_OK;
	}
	return NW_SUSPEND;
...
int psip_open( int port, int srfd, get_userdata_t get_userdata,
...
int psip_write(int fd, mnx_size_t count)
{
	...
	pack= (*psip_fd->pf_get_userdata)(psip_fd->pf_srfd, (mnx_size_t)0,
		count, FALSE);
	if (pack == NULL){
		pack= (*psip_fd->pf_get_userdata)(psip_fd->pf_srfd, 
			(mnx_size_t)EFAULT, (mnx_size_t)0, FALSE);
		assert(pack == NULL);
		return NW_OK;
	}
	...
	if (psip_fd->pf_flags & PFF_NEXTHOP){
		...
		if (pack_len <= sizeof(nexthop)){
			...
			pack= (*psip_fd->pf_get_userdata)(psip_fd->pf_srfd,
				(mnx_size_t)EMOLPACKSIZE, (mnx_size_t)0, FALSE);
			assert(pack == NULL);
			return NW_OK;
		}
		pack= bf_packIffLess(pack, sizeof(nexthop));
		nexthop= *(mnx_ipaddr_t *)ptr2acc_data(pack);
	...
	pack= (*psip_fd->pf_get_userdata)(psip_fd->pf_srfd, (mnx_size_t)count,
		(mnx_size_t)0, FALSE);
	assert(pack == NULL);
	return NW_OK;
}

int psip_select(int fd, unsigned operations)
{
...
int psip_cancel(int fd, int which_operation)
{
	...
	switch(which_operation)	{
		...
		case SR_CANCEL_READ:
			...
			psip_fd->pf_flags &= ~PFF_READ_IP;
			result= (*psip_fd->pf_put_userdata)(psip_fd->pf_srfd,
							(mnx_size_t)EMOLINTR, NULL, FALSE);
			assert(result == NW_OK);
			break;
