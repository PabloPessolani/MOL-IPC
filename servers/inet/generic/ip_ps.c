...
static int ipps_send( struct ip_port *ip_port, mnx_ipaddr_t dest, 
					acc_t *pack, int type );
...
void ipps_get(int ip_port_nr)
{
	...
	while (ip_port->ip_dl.dl_ps.ps_send_head != NULL){
		...
		dest= *(mnx_ipaddr_t *)ptr2acc_data(pack);
...
static int ipps_send(struct ip_port *ip_port, 
					mnx_ipaddr_t dest, acc_t *pack, int type)
{
	int result;
	acc_t *acc, *next_part;
