...
void udp_buffree( int priority );
#ifdef BUF_CONSISTENCY_CHECK
void udp_bufcheck( void );
#endif
void udp_main( udp_port_t *udp_port );
int udp_select( int fd, unsigned operations );
acc_t *udp_get_data( int fd, mnx_size_t offset, mnx_size_t count, 
	int for_ioctl );
int udp_put_data( int fd, mnx_size_t offset, acc_t *data, 	
	int for_ioctl );
int udp_peek( udp_fd_t * );
int udp_sel_read( udp_fd_t * );
void udp_restart_write_port( udp_port_t *udp_port );
void udp_ip_arrived( int port, acc_t *pack, mnx_size_t pack_size );
static void reply_thr_put( udp_fd_t *udp_fd, int reply,
	int for_ioctl );
static void reply_thr_get( udp_fd_t *udp_fd, int reply,
	int for_ioctl );
int udp_setopt( udp_fd_t *udp_fd );
static udpport_t find_unused_port( int fd );
static int is_unused_port( udpport_t port );
int udp_packet2user( udp_fd_t *udp_fd );
void restart_write_fd( udp_fd_t *udp_fd );
u16_t pack_oneCsum( acc_t *pack );
void udp_rd_enqueue( udp_fd_t *udp_fd, acc_t *pack,
							clock_t exp_tim );
static void hash_fd( udp_fd_t *udp_fd );
static void unhash_fd( udp_fd_t *udp_fd );
...
void udp_prep()
{
	...
	udp_port_table= inet_alloc(udp_conf_nr * sizeof(udp_port_table[0]));
}
...
void udp_init(void )
{
	...
	for (i= 0, udp_port= udp_port_table; i<udp_conf_nr; i++, udp_port++){
		...
		sr_add_minor(if2minor(ifno, UDP_DEV_MINOR),
...
int udp_open (int port, int srfd,get_userdata_t get_userdata,
...
int udp_ioctl (int fd, ioreq_t req)
{
	...
	switch(req)	{
		...
		case NWIOUDPPEEK:
			...
			break;
		default:
			reply_thr_get(udp_fd, EMOLBADIOCTL, TRUE);
			result= NW_OK;
			break;
	}
...
int udp_read (int fd, mnx_size_t count)
{
	...
	if (!(udp_fd->uf_flags & UFF_OPTSET)){
		reply_thr_put(udp_fd, EMOLBADMODE, FALSE);
		return NW_OK;
	}
...
acc_t *udp_get_data (int port, mnx_size_t offset,mnx_size_t count,int for_ioctl)
{
	udp_port_t *udp_port;
	udp_fd_t *udp_fd;
	int result;
...
int udp_put_data (int fd, mnx_size_t offset,acc_t *data,int for_ioctl)
{
	udp_port_t *udp_port;
	int result;
...
int udp_setopt(udp_fd_t *udp_fd)
{
	...
	if (new_en_flags & new_di_flags){
		DBLOCK(1, printf("returning EMOLBADMODE\n"));

		reply_thr_get(udp_fd, EMOLBADMODE, TRUE);
		return NW_OK;
	}
...
static void reply_thr_put(udp_fd_t *udp_fd,int reply,int for_ioctl)
{
	int result;
...
int udp_peek (udp_fd_t *udp_fd)
{
	...
	if (!(udp_fd->uf_flags & UFF_OPTSET)){
		udp_fd->uf_flags &= ~UFF_IOCTL_IP;
		reply_thr_put(udp_fd, EMOLBADMODE, TRUE);
		return NW_OK;
	}
...
int udp_sel_read (udp_fd_t *udp_fd)
{
	acc_t *pack, *tmp_acc, *next_acc;
	int result;
...
void udp_ip_arrived(int port, acc_t *pack, mnx_size_t pack_size)
{
	...
	mnx_size_t ip_hdr_size, udp_size, data_size, opt_size;
	mnx_ipaddr_t src_addr, dst_addr, ipaddr;
	...
	ip_hdr= (mnx_ip_hdr_t *)ptr2acc_data(ip_hdr_acc);
	...
	if (udp_hdr->uh_chksum){
		...
		chksum= oneC_sum(chksum, (u16_t *)&src_addr, sizeof(mnx_ipaddr_t));
		chksum= oneC_sum(chksum, (u16_t *)&dst_addr, sizeof(mnx_ipaddr_t));
...
#endif
		tmp_acc= bf_cut(ip_hdr_acc, (mnx_size_t)IP_MIN_HDR_SIZE, opt_size);
...
int udp_write(int fd, mnx_size_t count)
{
	...
	if (!(udp_fd->uf_flags & UFF_OPTSET))	{
		reply_thr_get (udp_fd, EMOLBADMODE, FALSE);
		return NW_OK;
	}
...
void restart_write_fd(udp_fd_t *udp_fd)
{
	...
	udp_io_hdr_t *udp_io_hdr;
	mnx_ip_hdr_t *ip_hdr;
	mnx_size_t ip_opt_size, user_data_size;
	unsigned long flags;
	u16_t chksum;
	u8_t u16[2];
	int result;
	...
	ip_hdr= (mnx_ip_hdr_t *)ptr2acc_data(ip_hdr_pack);
	...
	chksum= oneC_sum(chksum, (u16_t *)&udp_fd->uf_port->up_ipaddr,
		sizeof(mnx_ipaddr_t));
	chksum= oneC_sum(chksum, (u16_t *)&ip_hdr->ih_dst, sizeof(mnx_ipaddr_t));
	u16[0]= 0;
	u16[1]= IPPROTO_UDP;
	chksum= oneC_sum(chksum, (u16_t *)u16, sizeof(u16));
...
int udp_cancel(int fd, int which_operation)
{
	...
	switch (which_operation){
		case SR_CANCEL_READ:
			...
			udp_fd->uf_flags &= ~UFF_READ_IP;
			reply_thr_put(udp_fd, EMOLINTR, FALSE);
			break;
		case SR_CANCEL_WRITE:
			...
			reply_thr_get(udp_fd, EMOLINTR, FALSE);
			break;
		case SR_CANCEL_IOCTL:
			assert (udp_fd->uf_flags & UFF_IOCTL_IP);
			udp_fd->uf_flags &= ~UFF_IOCTL_IP;
			udp_fd->uf_flags &= ~UFF_PEEK_IP;
			reply_thr_get(udp_fd, EMOLINTR, TRUE);
			break;
		default:
...
void udp_rd_enqueue(udp_fd_t *udp_fd,acc_t *pack,clock_t exp_tim)
{
	acc_t *tmp_acc;
	int result;
