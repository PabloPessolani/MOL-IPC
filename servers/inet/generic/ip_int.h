...
typedef int (*ip_dev_send_t)( struct ip_port *ip_port, mnx_ipaddr_t dest, 
						acc_t *pack, int type );
...
typedef struct ip_port
{
	...
	mnx_ipaddr_t ip_ipaddr;

	mnx_ipaddr_t ip_subnetmask;
	mnx_ipaddr_t ip_classfulmask;
	u16_t ip_frame_id;
	u16_t ip_mtu;
...
#define IPPORT_FORMAT "ip_port=%d ip_flags=%d ip_dl_type=%X ip_ipaddr=%lX ip_subnetmask=%lX ip_mtu_max=%d\n"
#define IPPORT_FIELDS(p) 	p->ip_port,p->ip_flags,p->ip_dl_type, p->ip_ipaddr, p->ip_subnetmask, p->ip_mtu_max
...
typedef struct ip_ass
{
	...
	mnx_ipaddr_t ia_srcaddr, ia_dstaddr;
	int ia_proto, ia_id;
} ip_ass_t;
...
/* ip_lib.c */
mnx_ipaddr_t ip_get_netmask( mnx_ipaddr_t hostaddr );
mnx_ipaddr_t ip_get_ifaddr( int ip_port_nr );
int ip_chk_hdropt( u8_t *opt, int optlen );
void ip_print_frags( acc_t *acc );
nettype_t ip_nettype( mnx_ipaddr_t ipaddr );
mnx_ipaddr_t ip_netmask( nettype_t nettype );
char *ip_nettoa( nettype_t nettype );

/* ip_ps.c */
int ipps_init( ip_port_t *ip_port );
void ipps_get( int ip_port_nr );
void ipps_put( int ip_port_nr, mnx_ipaddr_t nexthop, acc_t *pack );

/* ip_read.c */
void ip_port_arrive( ip_port_t *port, acc_t *pack, mnx_ip_hdr_t *ip_hdr );
