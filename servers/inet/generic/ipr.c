...
{
	mnx_ipaddr_t orh_addr;
	oroute_t *orh_route;
} oroute_hash_t;

oroute_t oroute_table[OROUTE_NR];
oroute_t *oroute_head;
int static_oroute_nr;
oroute_hash_t oroute_hash_table[OROUTE_HASH_NR][OROUTE_HASH_ASS_NR];
...
{
	mnx_ipaddr_t irh_addr;
	iroute_t *irh_route;
} iroute_hash_t;

iroute_t iroute_table[IROUTE_NR];
iroute_hash_t iroute_hash_table[IROUTE_HASH_NR][IROUTE_HASH_ASS_NR];

static oroute_t *oroute_find_ent( int port_nr, mnx_ipaddr_t dest );
static void oroute_del( oroute_t *oroute );
static oroute_t *sort_dists( oroute_t *oroute );
static oroute_t *sort_gws( oroute_t *oroute );
static void oroute_uncache_nw( mnx_ipaddr_t dest, mnx_ipaddr_t netmask );
static void iroute_uncache_nw( mnx_ipaddr_t dest, mnx_ipaddr_t netmask );
...
int oroute_frag(int port_nr, mnx_ipaddr_t dest, int ttl, mnx_size_t msgsize, mnx_ipaddr_t *nexthop)
{
	oroute_t *oroute;

	SVRDEBUG("port_nr=%d dest=%08lX ttl=%d msgsize=%d\n",
		port_nr, dest, ttl, msgsize);
...
int ipr_add_oroute(int port_nr, mnx_ipaddr_t dest, mnx_ipaddr_t subnetmask,
			mnx_ipaddr_t gateway, time_t timeout, int dist, int mtu,
			int static_route, i32_t preference, oroute_t **oroute_p)
{
	int i;
	ip_port_t *ip_port;
...
void ipr_destunrch(int port_nr, mnx_ipaddr_t dest, mnx_ipaddr_t netmask, time_t timeout)
{
	oroute_t *oroute;
	int result;
...
void ipr_redirect(int port_nr, mnx_ipaddr_t dest, mnx_ipaddr_t netmask,
...
void ipr_ttl_exc(int port_nr, mnx_ipaddr_t dest, mnx_ipaddr_t netmask, time_t timeout)
{
	oroute_t *oroute;
	int new_dist;
	int result;
...
static oroute_t *oroute_find_ent(int port_nr, mnx_ipaddr_t dest)
{
	...
	SVRDEBUG("port_nr=%d dest=%08lX hash=%d\n", port_nr, dest, hash);
