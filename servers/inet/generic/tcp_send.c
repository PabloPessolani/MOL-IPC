...
#include "tcp_int.h"
 ...
 static void tcp_send_timeout( int conn, struct inet_timer *timer);
...
#if 0
void do_tcp_timeout(tcp_conn_t *tcp_conn)
{
...
void tcp_fd_write(tcp_conn_t *tcp_conn)
{
	...
	int urg, nourg, push;
	u32_t max_seq;
	mnx_size_t max_trans, write_count;
