...
int eth_ioctl(int fd, ioreq_t req)
{
	...
	switch (req)	{
		case NWIOSETHOPT: {
			...
			if (new_en_flags & new_di_flags)			{
				bf_afree(data);
				reply_thr_get (eth_fd, EMOLBADMODE, TRUE);
				return NW_OK;
			}	
	...
	case NWIOGETHSTAT:	{
			...
			if (!(eth_port->etp_flags & EPF_ENABLED))	{
				reply_thr_put(eth_fd, EMOLGENERIC, TRUE);
				return NW_OK;
			}
	...
	reply_thr_put(eth_fd, EMOLBADIOCTL, TRUE);
	return NW_OK;
}
...
int eth_write(int fd, mnx_size_t count)
{
	...
	if (!(eth_fd->ef_flags & EFF_OPTSET))	{
		reply_thr_get (eth_fd, EMOLBADMODE, FALSE);
		return NW_OK;
	}
	...
	if (count<ETH_MIN_PACK_SIZE || count>ETH_MAX_PACK_SIZE) {
		...
		reply_thr_get (eth_fd, EMOLPACKSIZE, FALSE);
		return NW_OK;
	}
...
int eth_read ( int fd, mnx_size_t count)
{
	...
	eth_fd= &eth_fd_table[fd];
	if (!(eth_fd->ef_flags & EFF_OPTSET)) 	{
		reply_thr_put(eth_fd, EMOLBADMODE, FALSE);
		return NW_OK;
	}
	if (count < ETH_MAX_PACK_SIZE) 	{
		reply_thr_put(eth_fd, EMOLPACKSIZE, FALSE);
		return NW_OK;
	}
...
int eth_cancel(int fd, int which_operation)
{
	...
	case SR_CANCEL_READ:
		assert (eth_fd->ef_flags & EFF_READ_IP);
		eth_fd->ef_flags &= ~EFF_READ_IP;
		reply_thr_put(eth_fd, EMOLINTR, FALSE);
		break;
	case SR_CANCEL_WRITE:
		...
		reply_thr_get(eth_fd, EMOLINTR, FALSE);
		break;
	case SR_CANCEL_IOCTL:
		assert (eth_fd->ef_flags & EFF_IOCTL_IP);
		eth_fd->ef_flags &= ~EFF_IOCTL_IP;
		reply_thr_get(eth_fd, EMOLINTR, TRUE);
		break;
	default:
