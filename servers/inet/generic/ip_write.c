...
int ip_write (int fd, mnx_size_t count)
{
	...
	if (count > IP_MAX_PACKSIZE){
		error_reply (ip_fd, EMOLPACKSIZE);
		return NW_OK;
	}
	pack= (*ip_fd->if_get_userdata)(ip_fd->if_srfd, (mnx_size_t)0,
		count, FALSE);
	if (!pack)
		return NW_OK;
...
int ip_send(int fd, acc_t *data, mnx_size_t data_len)
{
	ip_port_t *ip_port;
	ip_fd_t *ip_fd;
	mnx_ip_hdr_t *ip_hdr, *tmp_hdr;
	mnx_ipaddr_t dstaddr, nexthop, hostrep_dst, my_ipaddr, netmask;
	u8_t *addrInBytes;
	...
	if (data->acc_linkC != 1 || data->acc_buffer->buf_linkC != 1){
		tmp_pack= bf_memreq(IP_MIN_HDR_SIZE);
		tmp_hdr= (mnx_ip_hdr_t *)ptr2acc_data(tmp_pack);
		*tmp_hdr= *ip_hdr;
...
void ip_hdr_chksum(mnx_ip_hdr_t *ip_hdr, int ip_hdr_len)
{
	ip_hdr->ih_hdr_chk= 0;
...
acc_t *ip_split_pack (ip_port_t *ip_port, acc_t **ref_last, int mtu)
{
	...
	*ref_last= 0;
	...
	first_hdr= (mnx_ip_hdr_t *)ptr2acc_data(first_pack);
	...
	second_hdr= (mnx_ip_hdr_t *)ptr2acc_data(second_pack);
...
static void error_reply (ip_fd_t *ip_fd, int error)
{
	if ((*ip_fd->if_get_userdata)(ip_fd->if_srfd, (mnx_size_t)error,
		(mnx_size_t)0, FALSE)){
