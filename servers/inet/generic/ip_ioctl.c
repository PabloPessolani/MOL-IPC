...
static void reply_thr_get( ip_fd_t *ip_fd, mnx_size_t
	reply, int for_ioctl );
...
int ip_ioctl (int fd, ioreq_t req)
{
	...
	switch (req) 	{
		case NWIOSIPOPT:
			...
			if (new_en_flags & new_di_flags){
				bf_afree(data);
				reply_thr_get (ip_fd, EMOLBADMODE, TRUE);
				return NW_OK;
			}
		...
		case NWIOSIPCONF:
			ip_port= ip_fd->if_port;
			SVRDEBUG("ip_port:%d req=%X \n", ip_port->ip_port, req );

			if (req == NWIOSIPCONF2){
				SVRDEBUG("req=NWIOSIPCONF2\n");
				data= (*ip_fd->if_get_userdata)(ip_fd->if_srfd, 0, 
					sizeof(*ipconf2), TRUE);
				data= bf_packIffLess (data, sizeof(*ipconf2));
				SVRDEBUG("req=NWIOSIPCONF2 acc_length=%d\n", data->acc_length );
				...
				SVRDEBUG("req=NWIOSIPCONF\n");
				data= (*ip_fd->if_get_userdata)(ip_fd->if_srfd, 0, 
					sizeof(*ipconf), TRUE);
				data= bf_packIffLess (data, sizeof(*ipconf));
				SVRDEBUG("req=NWIOSIPCONF acc_length=%d sizeof(*ipconf)=%d\n", data->acc_length, sizeof(*ipconf) );
		...
		case NWIOGIPCONF:
			...
			SVRDEBUG(IPCONF_FORMAT, IPCONF_FIELDS(ipconf));
	...
	DBLOCK(1, printf("replying EMOLBADIOCTL: 0x%x\n", req));
	return (*ip_fd->if_put_userdata)(ip_fd->if_srfd, EMOLBADIOCTL,
		(acc_t *)0, TRUE);
}
...
int ip_setconf(int ip_port_nr,nwio_ipconf_t *ipconf)
{
	...
	SVRDEBUG("ip_port_nr=%d  nwic_flags=%X \n",
		ip_port_nr, ipconf->nwic_flags );
...
static void reply_thr_get(ip_fd_t *ip_fd,mnx_size_t reply,int for_ioctl)
{
	acc_t *result;
