...
static ip_ass_t *find_ass_ent( ip_port_t *ip_port, u16_t id,
	ipproto_t proto, mnx_ipaddr_t src, mnx_ipaddr_t dst );
static acc_t *merge_frags( acc_t *first, acc_t *second );
static int ip_frag_chk( acc_t *pack );
static  acc_t *reassemble( ip_port_t *ip_port, acc_t *pack, 
	mnx_ip_hdr_t *ip_hdr );
static void route_packets( event_t *ev, ev_arg_t ev_arg );
static int broadcast_dst( ip_port_t *ip_port, mnx_ipaddr_t dest );
...
int ip_read (int fd, mnx_size_t count)
{
	...
	if (!(ip_fd->if_flags & IFF_OPTSET)){
		return (*ip_fd->if_put_userdata)(ip_fd->if_srfd, EMOLBADMODE,
			(acc_t *)0, FALSE);
	}
...
static  acc_t *reassemble (ip_port_t *ip_port,acc_t *pack,mnx_ip_hdr_t *pack_hdr)
{
	ip_ass_t *ass_ent;
	mnx_size_t pack_hdr_len, pack_data_len, pack_offset, tmp_offset;
	u16_t pack_flags_fragoff;
	acc_t *prev_acc, *curr_acc, *next_acc, *head_acc, *tmp_acc;
	mnx_ip_hdr_t *tmp_hdr;
	time_t first_time;
	...
	while(next_acc){
		tmp_hdr= (mnx_ip_hdr_t *)ptr2acc_data(next_acc);
	...
	pack_hdr= (mnx_ip_hdr_t *)ptr2acc_data(pack);
	pack_flags_fragoff= ntohs(pack_hdr->ih_flags_fragoff);
...
static  acc_t *merge_frags (acc_t *first, acc_t *second)
{
	mnx_ip_hdr_t *first_hdr, *second_hdr;
	mnx_size_t first_hdr_size, second_hdr_size, first_datasize, second_datasize,
	...
	first_hdr= (mnx_ip_hdr_t *)ptr2acc_data(first);
	...
	second_hdr= (mnx_ip_hdr_t *)ptr2acc_data(second);
...
static  int ip_frag_chk(acc_t *pack)
{
	...
	if (pack->acc_length < sizeof(mnx_ip_hdr_t)){
...
void ip_packet2user (ip_fd_t *ip_fd, acc_t *pack, time_t exp_time, mnx_size_t data_len)
{
	acc_t *tmp_pack;
	mnx_ip_hdr_t *ip_hdr;
	int result, ip_hdr_len;
	mnx_size_t transf_size;
...
void ip_port_arrive (ip_port_t *ip_port, acc_t *pack, mnx_ip_hdr_t *ip_hdr)
{
...
void ip_arrived(ip_port_t *ip_port, acc_t *pack)
{
	mnx_ip_hdr_t *ip_hdr;
	mnx_ipaddr_t dest;
...
void ip_arrived_broadcast(ip_port_t *ip_port, acc_t *pack)
{
	mnx_ip_hdr_t *ip_hdr;
...
static  int broadcast_dst(ip_port_t *ip_port, mnx_ipaddr_t dest)
{
	mnx_ipaddr_t my_ipaddr, netmask, classmask;
	...
	if (dest == HTONL((mnx_ipaddr_t)-1))
		return 1;
#if IP_42BSD_BCAST
	if (dest == HTONL((mnx_ipaddr_t)0))
		return 1;
#endif
