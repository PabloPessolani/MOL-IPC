...
void tcp_main( tcp_port_t *port );
int tcp_select( int fd, unsigned operations );
acc_t *tcp_get_data( int fd, mnx_size_t offset,
	mnx_size_t count, int for_ioctl );
int tcp_put_data( int fd, mnx_size_t offset,
	acc_t *data, int for_ioctl );
void tcp_put_pkt( int fd, acc_t *data, mnx_size_t datalen );
void read_ip_packets( tcp_port_t *port );
int tcp_setconf( tcp_fd_t *tcp_fd );
int tcp_setopt( tcp_fd_t *tcp_fd );
int tcp_connect( tcp_fd_t *tcp_fd );
int tcp_listen( tcp_fd_t *tcp_fd, int do_listenq );
int tcp_acceptto( tcp_fd_t *tcp_fd );
tcpport_t find_unused_port( int fd );
int is_unused_port( tcpport_t port );
static int reply_thr_put( tcp_fd_t *tcp_fd, int reply,
	int for_ioctl );
static void reply_thr_get( tcp_fd_t *tcp_fd, int reply,
	int for_ioctl );
tcp_conn_t *find_conn_entry( tcpport_t locport,
	mnx_ipaddr_t locaddr, tcpport_t remport, mnx_ipaddr_t readaddr );
tcp_conn_t *find_empty_conn( void );
tcp_conn_t *find_best_conn( mnx_ip_hdr_t *ip_hdr, 
	tcp_hdr_t *tcp_hdr );
tcp_conn_t *new_conn_for_queue( tcp_fd_t *tcp_fd );
int maybe_listen( mnx_ipaddr_t locaddr, tcpport_t locport,
				mnx_ipaddr_t remaddr, tcpport_t remport );
int tcp_su4connect( tcp_fd_t *tcp_fd );
void tcp_buffree( int priority );
#ifdef BUF_CONSISTENCY_CHECK
void tcp_bufcheck( void );
#endif
void tcp_setup_conn( tcp_port_t *tcp_port,
					tcp_conn_t *tcp_conn );
u32_t tcp_rand32( void );

void tcp_prep(void)
{
	SVRDEBUG("\n");

	tcp_port_table= inet_alloc(tcp_conf_nr * sizeof(tcp_port_table[0]));
}
...
void tcp_init(void)
{
	...
	for (i=0, tcp_port= tcp_port_table; i<tcp_conf_nr; i++, tcp_port++)	{
		...
		sr_add_minor(if2minor(ifno, TCP_DEV_MINOR),
...
void tcp_main(tcp_port_t *tcp_port)
{
	int result, i;
	tcp_conn_t *tcp_conn;
	tcp_fd_t *tcp_fd;
...
acc_t *tcp_get_data (int port, mnx_size_t offset,mnx_size_t count,int for_ioctl)
{
	tcp_port_t *tcp_port;
	int result;
...
int tcp_put_data (int fd, mnx_size_t offset,acc_t *data,int for_ioctl)
{
	tcp_port_t *tcp_port;
	int result;
...
void tcp_put_pkt(int fd, acc_t *data,mnx_size_t datalen)
{
	...
	mnx_size_t ip_datalen, tcp_datalen, ip_hdr_len, tcp_hdr_len;
	u16_t sum, mtu;
	u32_t bits;
	int i, hash;
	mnx_ipaddr_t srcaddr, dstaddr, ipaddr, mask;
...
int tcp_open (int port,int srfd,get_userdata_t get_userdata,
...
int tcp_ioctl (int fd, ioreq_t req)
{
	...
	switch (req){
		...
		default:
			tcp_fd->tf_flags &= ~TFF_IOCTL_IP;
			reply_thr_get(tcp_fd, EMOLBADIOCTL, TRUE);
			result= NW_OK;
			break;
	}
	return result;
...
int tcp_setconf(tcp_fd_t *tcp_fd)
{
	...
	if (new_en_flags & new_di_flags){
		tcp_fd->tf_flags &= ~TFF_IOCTL_IP;
		reply_thr_get(tcp_fd, EMOLBADMODE, TRUE);
		bf_afree(data);
		return NW_OK;
	}
...
tcp_conn_t *find_empty_conn(void )
{
	...
	for (i=tcp_conf_nr, tcp_conn= tcp_conn_table+i;
		...
		if (tcp_conn->tc_senddis > get_time())
			continue;
		if (tcp_conn->tc_state != TCS_CLOSED){
			 tcp_close_connection (tcp_conn, EMOLNOCONN);
		}
		tcp_conn->tc_flags= 0;
		return tcp_conn;
	}
...
tcp_conn_t *find_conn_entry(tcpport_t locport, 
	mnx_ipaddr_t locaddr,tcpport_t remport,mnx_ipaddr_t remaddr)
{
	tcp_conn_t *tcp_conn;
	int i, state;
...
void read_ip_packets(tcp_port_t *tcp_port)
{
	int result;
...
tcp_conn_t *find_best_conn(mnx_ip_hdr_t *ip_hdr, tcp_hdr_t *tcp_hdr)
{
	...
	tcp_fd_t *tcp_fd;
	int i;
	mnx_ipaddr_t locaddr;
	mnx_ipaddr_t remaddr;
	tcpport_t locport;
	tcpport_t remport;
	...
	if (best_conn){
		...
		if (best_conn->tc_state != TCS_CLOSED)
			tcp_close_connection(best_conn, EMOLNOCONN);
...
void tcp_reply_write(tcp_fd_t *tcp_fd, mnx_size_t reply)
{
...
void tcp_reply_read(tcp_fd_t *tcp_fd, mnx_size_t reply)
{
...
int tcp_write(int fd, mnx_size_t count)
{
	tcp_fd_t *tcp_fd;
	tcp_conn_t *tcp_conn;
...
int tcp_read(int fd, mnx_size_t count)
{
	tcp_fd_t *tcp_fd;
	tcp_conn_t *tcp_conn;
	...
	switch (which_operation){
		case SR_CANCEL_WRITE:
			...
			if (tcp_fd->tf_write_offset)
				...
				reply_thr_get (tcp_fd, EMOLINTR, FALSE);
			break;
		case SR_CANCEL_READ:
			assert (tcp_fd->tf_flags & TFF_READ_IP);
			tcp_fd->tf_flags &= ~TFF_READ_IP;
			if (tcp_fd->tf_read_offset)
				...
				reply_thr_put (tcp_fd, EMOLINTR, FALSE);
			break;
		case SR_CANCEL_IOCTL:
			...
			if (tcp_fd->tf_flags & TFF_IOC_INIT_SP)	{
				tcp_fd->tf_flags &= ~TFF_IOC_INIT_SP;
				reply_thr_put (tcp_fd, EMOLINTR, TRUE);
				break;
			}
			...
			switch (tcp_fd->tf_ioreq)	{
				...
				case NWIOSTCPCONF:
				case NWIOTCPSHUTDOWN:
					reply_thr_get (tcp_fd, EMOLINTR, TRUE);
					break;
				case NWIOTCPCONN:
				case NWIOTCPLISTEN:
					assert (tcp_conn->tc_connInprogress);
...
int tcp_connect(tcp_fd_t *tcp_fd)
{
	...
	if (!(tcp_fd->tf_flags & TFF_CONF_SET))	{
		tcp_reply_ioctl(tcp_fd, EMOLBADMODE);
		return NW_OK;
	}
...
void tcp_buffree (int priority)
{
	...
	if (priority == TCP_PRI_CONNwoUSER){
		for (i=0, tcp_conn= tcp_conn_table; i<TCP_CONN_NR; i++,
			...
			tcp_close_connection (tcp_conn, EMOLOUTOFBUFS);
		}
	}
...
void tcp_notreach(tcp_conn_t *tcp_conn)
{
	...
	if (new_ttl == IP_MAX_TTL){
		if (tcp_conn->tc_state == TCS_SYN_SENT)
			tcp_close_connection(tcp_conn, EMOLDSTNOTRCH);
		return;
...
void tcp_setup_conn(tcp_port_t *tcp_port, tcp_conn_t *tcp_conn)
{
	u16_t mss;
