...
#define ICMPPORT_FORMAT "icp_flags=%X icp_state=%X icp_ipport=%d icp_ipfd=%d\n" 
#define ICMPPORT_FIELDS(p) p->icp_flags, p->icp_state, p->icp_ipport, p->icp_ipfd
...
icmp_port_t *icmp_port_table;

static void icmp_main( icmp_port_t *icmp_port );
static acc_t *icmp_getdata( int port, mnx_size_t offset,
	mnx_size_t count, int for_ioctl );
static int icmp_putdata( int port, mnx_size_t offset,
	acc_t *data, int for_ioctl );
static void icmp_read( icmp_port_t *icmp_port );
static void process_data( icmp_port_t *icmp_port,
	acc_t *data );
static u16_t icmp_pack_oneCsum( acc_t *ip_pack );

static void icmp_echo_request( icmp_port_t *icmp_port,
							acc_t *ip_pack, int ip_hdr_len, mnx_ip_hdr_t *ip_hdr,
	acc_t *icmp_pack, int icmp_len, icmp_hdr_t *icmp_hdr );
					
static void icmp_dst_unreach( icmp_port_t *icmp_port,
	acc_t *ip_pack, int ip_hdr_len, mnx_ip_hdr_t *ip_hdr,
	acc_t *icmp_pack, int icmp_len, icmp_hdr_t *icmp_hdr );
static void icmp_time_exceeded( icmp_port_t *icmp_port,
	acc_t *ip_pack, int ip_hdr_len, mnx_ip_hdr_t *ip_hdr,
	acc_t *icmp_pack, int icmp_len, icmp_hdr_t *icmp_hdr );
static void icmp_router_advertisement( icmp_port_t *icmp_port,
	acc_t *icmp_pack, int icmp_len, icmp_hdr_t *icmp_hdr );
static void icmp_redirect( icmp_port_t *icmp_port,
	mnx_ip_hdr_t *ip_hdr, acc_t *icmp_pack, int icmp_len,
	icmp_hdr_t *icmp_hdr );
static acc_t *make_repl_ip( mnx_ip_hdr_t *ip_hdr,
	int ip_len );
static void enqueue_pack( icmp_port_t *icmp_port,
	acc_t *reply_ip_hdr );
static int icmp_rate_limit( icmp_port_t *icmp_port,
	acc_t *reply_ip_hdr );
static void icmp_write( event_t *ev, ev_arg_t ev_arg );
static void icmp_buffree( int priority );
static acc_t *icmp_err_pack( acc_t *pack, icmp_hdr_t **icmp_hdr_pp );
#ifdef BUF_CONSISTENCY_CHECK
void icmp_bufcheck( void );
#endif

void icmp_prep(void )
{
	icmp_port_table= inet_alloc(ip_conf_nr * sizeof(icmp_port_table[0]));
}
...
static acc_t *icmp_getdata(int port, mnx_size_t offset, mnx_size_t count, int for_ioctl)
{
	icmp_port_t *icmp_port;
	nwio_ipopt_t *ipopt;
	acc_t *data;
	...
	SVRDEBUG("port=%d offset=%d count=%d for_ioctl=%d sizeof(*ipopt)=%d\n",
		port, offset, count,  for_ioctl, sizeof(*ipopt));

	icmp_port= &icmp_port_table[port];
	SVRDEBUG(ICMPPORT_FORMAT, ICMPPORT_FIELDS(icmp_port));
	...
	if (icmp_port->icp_flags & ICPF_WRITE_IP){
		if (!count)	{
			...
			return NW_OK; // como retorna OK si debe retornar static acc_t *
		}
	...
	switch (icmp_port->icp_state){
		case ICPS_IPOPT:
			if (!count)	{
				result= (int)offset;
//				assert(result == NW_OK); PAP: COMO puede hacer este assert si mas adelante compara result ?
				if (result < 0)	{
			...
			SVRDEBUG(IPOPT_FORMAT, IPOPT_FIELDS(ipopt));
			SVRDEBUG("data->acc_length=%d\n",data->acc_length);
			return data;
		default:
			break;
	}
...
static int icmp_putdata(int port, mnx_size_t offset, acc_t *data,int for_ioctl)
{
	icmp_port_t *icmp_port;
	int result;

	SVRDEBUG("port=%d offset=%d for_ioctl=%d\n",port, offset, for_ioctl );
...
void icmp_snd_redirect(int port_nr, acc_t *pack,int code, mnx_ipaddr_t gw)
{
	icmp_hdr_t *icmp_hdr;
	icmp_port_t *icmp_port;
...
static void process_data(icmp_port_t *icmp_port,acc_t *data)
{
	mnx_ip_hdr_t *ip_hdr;
	icmp_hdr_t *icmp_hdr;
	acc_t *icmp_dt;
	...
	ip_hdr= (mnx_ip_hdr_t *)ptr2acc_data(data);
	...
	icmp_dt= bf_cut(data, ip_hdr_len, pack_len);

	icmp_dt= bf_packIffLess (icmp_dt, ICMP_MIN_HDR_SIZE);
	icmp_hdr= (icmp_hdr_t *)ptr2acc_data(icmp_dt);

	if ((u16_t)~icmp_pack_oneCsum(icmp_dt)){
		DBLOCK(1, printf(
			"got packet with bad checksum (= 0x%x, 0x%x)\n",
			icmp_hdr->ih_chksum,
			(u16_t)~icmp_pack_oneCsum(icmp_dt)));
		bf_afree(data);
		bf_afree(icmp_dt);
		return;
	}
	...
	switch (icmp_hdr->ih_type){
		...
		case ICMP_TYPE_REDIRECT:
			icmp_redirect (icmp_port, ip_hdr, icmp_dt, pack_len,
				icmp_hdr);
			break;
		...
		case ICMP_TYPE_ROUTER_ADVER:
			icmp_router_advertisement(icmp_port, icmp_dt, pack_len, 
				icmp_hdr);
			break;
...
static void icmp_echo_request(icmp_port_t *icmp_port, acc_t *ip_data, int ip_len,
						mnx_ip_hdr_t *ip_hdr, acc_t *icmp_dt,
						int icmp_len, icmp_hdr_t *icmp_hdr)
{
	acc_t *repl_ip_hdr, *repl_icmp;
	mnx_ipaddr_t tmpaddr, locaddr, netmask;
	icmp_hdr_t *repl_icmp_hdr;
	i32_t tmp_chksum;
	ip_port_t *ip_port;

	SVRDEBUG("icp_ipport=%d ip_len=%d  icmp_len=%d\n",
		icmp_port->icp_ipport, ip_len, icmp_len );
	...
	repl_icmp->acc_next= bf_cut (icmp_dt, ICMP_MIN_HDR_SIZE,
...
static u16_t icmp_pack_oneCsum(acc_t *icmp_pack)
{
	u16_t prev;
	int odd_byte;
	char *data_ptr;
...
static acc_t *make_repl_ip(mnx_ip_hdr_t *ip_hdr,int ip_len)
{
	mnx_ip_hdr_t *repl_ip_hdr;
	acc_t *repl;
	int repl_hdr_len;
	...
	repl_ip_hdr= (mnx_ip_hdr_t *)ptr2acc_data(repl);
...
static void enqueue_pack(icmp_port_t *icmp_port, acc_t *reply_ip_hdr)
{
	int r;
	ev_arg_t ev_arg;
...
static int icmp_rate_limit(icmp_port_t *icmp_port, acc_t *reply_ip_hdr)
{
	time_t t;
	acc_t *pack;
	mnx_ip_hdr_t *ip_hdr;
	...
	ip_hdr= (mnx_ip_hdr_t *)ptr2acc_data(pack);
	...
	switch(type){
		...
		case ICMP_TYPE_PARAM_PROBLEM:
			...
			ip_hdr= (mnx_ip_hdr_t *)(ptr2acc_data(pack)+hdrlen+icmp_hdr_len);
			icmp_hdr= (icmp_hdr_t *)(ptr2acc_data(pack)+hdrlen);
...
#ifdef BUF_CONSISTENCY_CHECK
static void icmp_bufcheck(void )
{
	int i;
	icmp_port_t *icmp_port;
	acc_t *pack;
...
static void icmp_dst_unreach(icmp_port_t *icmp_port, acc_t *ip_pack, 
				int ip_hdr_len, mnx_ip_hdr_t *ip_hdr, acc_t *icmp_pack,
				int icmp_len, icmp_hdr_t *icmp_hdr)
{
	acc_t *old_ip_pack;
	mnx_ip_hdr_t *old_ip_hdr;
	int ip_port_nr;
	mnx_ipaddr_t dst, mask;
	mnx_size_t old_pack_size;
	u16_t new_mtu;

	SVRDEBUG("ip_hdr_len=%d icmp_len=%d \n", ip_hdr_len, icmp_len);
	...
	old_ip_hdr= (mnx_ip_hdr_t *)ptr2acc_data(old_ip_pack);
	...
	switch(icmp_hdr->ih_code){
		...
		case ICMP_HOST_UNRCH:
			ipr_destunrch (ip_port_nr, old_ip_hdr->ih_dst, (mnx_ipaddr_t)-1,
				IPR_UNRCH_TIMEOUT);
			break;
...
static void icmp_time_exceeded(icmp_port_t *icmp_port, acc_t *ip_pack, int ip_hdr_len,
				mnx_ip_hdr_t *ip_hdr, acc_t *icmp_pack, int icmp_len, icmp_hdr_t *icmp_hdr)
{
	acc_t *old_ip_pack;
	mnx_ip_hdr_t *old_ip_hdr;
	int ip_port_nr;
	...
	switch(icmp_hdr->ih_code){
		case ICMP_TTL_EXC:
			ipr_ttl_exc (ip_port_nr, old_ip_hdr->ih_dst, (mnx_ipaddr_t)-1,
				IPR_TTL_TIMEOUT);
			break;
...
static void icmp_router_advertisement(icmp_port_t *icmp_port, acc_t *icmp_pack,
					int icmp_len, icmp_hdr_t *icmp_hdr)
{
	int entries;
	int entry_size;
...
static void icmp_redirect(icmp_port_t *icmp_port, mnx_ip_hdr_t *ip_hdr,
				acc_t *icmp_pack, int icmp_len, icmp_hdr_t *icmp_hdr)
{
	acc_t *old_ip_pack;
	mnx_ip_hdr_t *old_ip_hdr;
	...
	switch(icmp_hdr->ih_code) {
		...
		case ICMP_REDIRECT_HOST:
			ipr_redirect (ip_port_nr, old_ip_hdr->ih_dst, (mnx_ipaddr_t)-1,
				ip_hdr->ih_src, icmp_hdr->ih_hun.ihh_gateway, 
				IPR_REDIRECT_TIMEOUT);
			break;
		default:
...
static acc_t *icmp_err_pack(acc_t *pack, icmp_hdr_t **icmp_hdr_pp)
{
	...
	mnx_size_t size, pack_len;
	mnx_ipaddr_t dest, netmask;
	nettype_t nettype;
	...
	*icmp_hdr_pp= icmp_hdr_p;
	...
	ip_hdr= (mnx_ip_hdr_t *)ptr2acc_data(ip_pack);
