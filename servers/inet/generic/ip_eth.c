...
{
	time_t xh_time;
	mnx_ipaddr_t xh_ipaddr;
} xmit_hdr_t;

static mnx_ethaddr_t broadcast_ethaddr=
{
	{ 0xff, 0xff, 0xff, 0xff, 0xff, 0xff }
};
static mnx_ethaddr_t ipmulticast_ethaddr=
{
	{ 0x01, 0x00, 0x5e, 0x00, 0x00, 0x00 }
};

static void do_eth_read( ip_port_t *port );
static acc_t *get_eth_data( int fd, mnx_size_t offset,
	mnx_size_t count, int for_ioctl );
static int put_eth_data( int fd, mnx_size_t offset,
	acc_t *data, int for_ioctl );
static void ipeth_main( ip_port_t *port );
static void ipeth_set_ipaddr( ip_port_t *port );
static void ipeth_restart_send( ip_port_t *ip_port );
static int ipeth_send( struct ip_port *ip_port, mnx_ipaddr_t dest, 
	acc_t *pack, int type );
static void ipeth_arp_reply( int ip_port_nr, mnx_ipaddr_t ipaddr,
	mnx_ethaddr_t *dst_ether_ptr );
static int ipeth_update_ttl( time_t enq_time, time_t now,
	acc_t *eth_pack );
static void ip_eth_arrived( int port, acc_t *pack,
	mnx_size_t pack_size );
...
static void ipeth_main(ip_port_t *ip_port)
{
	int result;
...
static acc_t *get_eth_data (int fd, mnx_size_t offset,mnx_size_t count,int for_ioctl)
{
	ip_port_t *ip_port;
	acc_t *data;
	int result;
...
static int put_eth_data (int port, mnx_size_t offset,acc_t *data,int for_ioctl)
{
	ip_port_t *ip_port;
	int result;
...
static void ipeth_set_ipaddr(ip_port_t *ip_port)
{
	SVRDEBUG("port=%d\n",ip_port->ip_port); 
...
static int ipeth_send(struct ip_port *ip_port,mnx_ipaddr_t dest,acc_t *pack,int type)
{
	int i, r;
	acc_t *eth_pack, *tail;
	mnx_size_t pack_size;
	eth_hdr_t *eth_hdr;
	xmit_hdr_t *xmit_hdr;
	mnx_ipaddr_t hostpart, tmpaddr;
	time_t t;
	u32_t *p;

	SVRDEBUG("dest=%08lX type=%X\n", dest, type);
...
static void ipeth_arp_reply(int ip_port_nr, mnx_ipaddr_t ipaddr,mnx_ethaddr_t *eth_addr)
{
	acc_t *prev, *eth_pack;
	int r;
	xmit_hdr_t *xmit_hdr;
...
static int ipeth_update_ttl(time_t enq_time, time_t now, acc_t *eth_pack)
{
	int ttl_diff;
	mnx_ip_hdr_t *ip_hdr;
	u32_t sum;
...
static void do_eth_read(ip_port_t *ip_port)
{
	int result;
...
static void ip_eth_arrived(int port, acc_t *pack, mnx_size_t pack_size)
{
	int broadcast;
	ip_port_t *ip_port;
