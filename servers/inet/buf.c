...
#define ALLOC_STORAGE(Ident, Nitems, Label)				\
	...
	{								\
		...
		posix_memalign( (void **) Ident, getpagesize(), (sizeof(*Ident) * Nitems) );\
		if (!Ident)						\
...
unsigned buf_generation; 
 bf_checkreq_t checkreq[CLIENT_NR];
#endif

#ifndef BUF_TRACK_ALLOC_FREE
 acc_t *bf_small_memreq ( mnx_size_t size );
#else
 acc_t *_bf_small_memreq ( char *clnt_file, int clnt_line,
								mnx_size_t size );
#define bf_small_memreq(a) _bf_small_memreq(clnt_file, clnt_line, a)
#endif
 void free_accs ( void );
#ifdef BUF_CONSISTENCY_CHECK
 void count_free_bufs ( acc_t *list );
 int report_buffer ( buf_t *buf, char *label, int i );
#endif
...
#ifndef BUF_TRACK_ALLOC_FREE
 acc_t *bf_small_memreq(size)
#else
 acc_t *_bf_small_memreq(clnt_file, clnt_line, size)
char *clnt_file;
int clnt_line;
#endif
mnx_size_t size;
...
register acc_t *acc_ptr;
{
 ...
 mnx_size_t bf_bufsize(acc_ptr)
register acc_t *acc_ptr;
{
	register mnx_size_t size;
...
acc_t *old_acc;
{
	...
	mnx_size_t size, offset_old, offset_new, block_size, block_size_old;
...
acc_t  *data_second;
{
	...
	mnx_size_t size, offset_old, offset_new, block_size_old, block_size;
