...
/* Check if a device exists with the proper device number. */
{
	...
	SVRDEBUG("device=%s mode=%X minor=%d\n", device,  mode,  minor);
	...
	while (token(0), word[0] != 0) {
		if (strncmp(word, "eth", 3) == 0) {
			...
			if (strcmp(word, "vlan") == 0) {
				...
				ecp->ec_task= inet_alloc(strlen(word)+1);
...
void *inet_alloc(mnx_size_t size)
{
	...
	asize = ((size + (sizeof(char *) - 1)) & ~(sizeof(char *) - 1));
	posix_memalign( (void **) &ptr, getpagesize(), asize );
	if (ptr== NULL) {
   		ERROR_EXIT(errno);
	}
