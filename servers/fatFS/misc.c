 ...
 */
  ...
  if ((f = get_filp(rfd)) == NIL_FILP) ERROR_RETURN(err_code);
  ...
  /* dup2(fd, fd2) */
  if (m_in.fd2 < 0 || m_in.fd2 >= MNX_OPEN_MAX) ERROR_RETURN(EMOLBADF);
  ...
  MOL_FD_SET(m_in.fd2, &fp->fp_filp_inuse);
  return(m_in.fd2);
}
...
int do_fcntl()
{
  ...
  switch (m_in.request) {
      ...
      SVRDEBUG("m_in.fd is=%d\n", m_in.fd);
      SVRDEBUG("((fp->fp_cloexec >> m_in.fd) & 01)%d\n", ((fp->fp_cloexec >> m_in.fd) & 01));      
