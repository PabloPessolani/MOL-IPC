 ...
 * signature.
 *
 *Inicializa la estructura que contiene el estado de computar MD5(RFC 1321, 3.3:Step 3)
 *Esto es sólo para los cálculos progresivos de MD5. Si se tiene la cadena de string completa
 *disponible, deberá utilizar md5_buffer.
 *Deberá llamar a md5_process, para cada grupo de bytes y después de la última llamada de proceso,
 *debe llamarse a md5_finish para obtener la "firma" (signature)
 *
 * RETURNS:
 *
 * None.
 ...
 *
 * Esta función es usada para calcular progresivamente la "firma" MD5 como 
 * un número de bytes a la vez. Si se tiene el string completo disponible, deberá usar md5_buffer. 
 * La estructura MD5 que se debió haber inicializado con md5_init y después de llamar al último proceso, debería llamarse
 * a md5_finish para obtener el resultado.
 *
 * RETURNS:
 *
 * None.
 ...
 *
 * Finaliza un cálculo progresivo de MD5 y copia el resultado "firma" de MD5 en el 
 * buffer resultado que debería ser de 16 bytes (MD5_SIZE). Después de esta llamada,
 * la estructura MD5 es inválida.
 *
 * RETURNS:
 *
 * None.
 ...
 *
 * Esta función es usada para calcular una firma MD5 para un buffer de bytes.
 * Si solamente tienen una parte de un buffer que desea procesar entonces
 * md5_init, md5_process, y md5_finish deberían usare.
 *
 * RETURNS:
 *
 * None.
 ...
 * representation.
 *
 * Convierte una firma MD5 en un buffer de 16 bytes a una representación de un string hexadecimal.
 ...
 * a 16 byte buffer.
 *
 * Convierte una firma MD5  desde una representación de string hexadecimal a un buffer de 16 bytes-
 *
 * RETURNS:
 *
 * None.
