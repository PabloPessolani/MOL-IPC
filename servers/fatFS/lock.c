...
/* Perform the advisory locking required by POSIX. */
 ...
 //  r = sys_datacopy(who_e, (vir_bytes) user_flock,
  ...
  r = mnx_vcopy(who_e, user_flock, FS_PROC_NR, &flock, sizeof(flock));    
  if (r != OK) ERROR_RETURN(EMOLINVAL);

  /* Make some error checks. */
  ltype = flock.l_type;
  mo = f->filp_mode;
  if (ltype != F_UNLCK && ltype != F_RDLCK && ltype != F_WRLCK) ERROR_RETURN(EMOLINVAL);
  if (req == F_GETLK && ltype == F_UNLCK) ERROR_RETURN(EINVAL);
  if ( (f->filp_ino->i_mode & I_TYPE) != I_REGULAR) ERROR_RETURN(EMOLINVAL);
  if (req != F_GETLK && ltype == F_RDLCK && (mo & R_BIT) == 0) ERROR_RETURN(EMOLBADF);
  if (req != F_GETLK && ltype == F_WRLCK && (mo & W_BIT) == 0) ERROR_RETURN(EMOLBADF);
  ...
  if (last < first) ERROR_RETURN(EMOLINVAL);
  ...
  for (flp = &file_lock[0]; flp < & file_lock[NR_LOCKS]; flp++) {
	...
	if (nr_locks == NR_LOCKS) ERROR_RETURN(EMOLNOLCK);
  ...
  if (req == F_GETLK) {
	...
	// 	who_e, (vir_bytes) user_flock, (phys_bytes) sizeof(flock));
 	r = mnx_vcopy(FS_PROC_NR, &flock, who_e, user_flock, sizeof(flock));	
	return(r);
  }
  ...
  if (empty == (struct file_lock *) 0) ERROR_RETURN(EMOLNOLCK);	/* table full */
 ...
 */
  ...
  for (fptr = &fproc[INIT_PROC_NR + 1]; fptr < &fproc[dc_ptr->dc_nr_procs]; fptr++){
