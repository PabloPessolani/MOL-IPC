 ...
 */
	...
	*k = -1;			/* we need a way to tell if file desc found */
	...
	for (i = start; i < MNX_OPEN_MAX; i++) {
		if (fp->fp_filp[i] == NIL_FILP && !MOL_FD_ISSET(i, &fp->fp_filp_inuse)) {
  ...
  if (*k < 0) ERROR_RETURN(EMOLMFILE);	/* this is why we initialized k to -1 */
...
/* See if 'fild' refers to a valid file descr.  If so, return its filp ptr. */
  ...
  // SVRDEBUG("ESTE FD %d IS SET?????=%d\n", fild, MOL_FD_ISSET(fild, &fp->fp_filp_inuse));
