...
/* Perform the mount(name, mfile, rd_only) system call. */
  ...
  if (!super_user) ERROR_RETURN(EMOLPERM);

  /* If 'name' is not for a block special file, return error. */
  if (fetch_name(m_in.name1, m_in.name1_length, M1) != OK) ERROR_RETURN(err_code);
  if ( (dev = name_to_dev(user_path)) == NO_DEV) ERROR_RETURN(err_code);
...
"do_mount: s_imount = 0x%x (%x, %d), s_isup = 0x%x (%x, %d), fp_rootdir = 0x%x\n",
    ...
    xp->s_isup == fproc[FS_PROC_NR].fp_rootdir)
  {
     ...
     * is in use.
     */
    ERROR_RETURN(EMOLBUSY);  /* already mounted */
  }
  ...
  if (sp == NIL_SUPER) ERROR_RETURN(EMOLNFILE);  /* no super block available */
  ...
  for (i= 0, tfp= fproc; i<dc_ptr->dc_nr_procs; i++, tfp++)
  {
  ...
  /* It may not be busy. */
  r = OK;
  if (rip->i_count > 1) r = EMOLBUSY;

  /* It may not be special. */
  bits = rip->i_mode & I_TYPE;
  if (bits == I_BLOCK_SPECIAL || bits == I_CHAR_SPECIAL) r = EMOLNOTDIR;
...
/* Unmount a file system by device number. */
  ...
  if (count > 1) ERROR_RETURN(EMOLBUSY); /* can't umount a busy file system */
