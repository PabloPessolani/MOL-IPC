...
#define SU_UID 	 ((mnx_uid_t) 0)	/* super_user's uid_t */
#define SERVERS_UID ((mnx_uid_t) 11) /* who may do FSSIGNON */
#define SYS_UID  ((mnx_uid_t) 0)	/* uid_t for processes MM and INIT */
#define SYS_GID  ((mnx_gid_t) 0)	/* gid_t for processes MM and INIT */
...
#define NO_BIT   ((mnx_bit_t) 0)	/* returned by alloc_bit() to signal failure */
...
#define BOOT_BLOCK  ((mnx_block_t) 0)	/* block number of boot block */
#define SUPER_BLOCK_BYTES (1024)	/* bytes offset */
#define START_BLOCK 	2		/* first block of FS (not counting SB) */

#define DIR_ENTRY_SIZE       usizeof (struct mnx_direct)  /* # bytes/dir entry   */
...
#define FS_BITCHUNK_BITS		(usizeof(bitchunk_t) * MNX_CHAR_BIT)
#define FS_BITS_PER_BLOCK(b)	(FS_BITMAP_CHUNKS(b) * FS_BITCHUNK_BITS)

/* Derived sizes pertaining to the V1 file system. */
#define V1_ZONE_NUM_SIZE           usizeof (mnx_zone1_t)  /* # bytes in V1 zone  */
...
#define V2_ZONE_NUM_SIZE            usizeof (mnx_zone_t)  /* # bytes in V2 zone  */
