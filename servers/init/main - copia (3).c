...
int main ( int argc, char *argv[] )
{
	...
	/* Bind to PM  */
	init_pid = mol_bindproc(INIT_PROC_EP, INIT_PID_NODE, init_lpid, REPLICA_BIND);
...
#define SINGLE_SERVER 	10 	/* endpoint of remote SINGLE SERVER */
			...
			ret = mnx_sendrec(SINGLE_SERVER , (long) m_ptr);
			if(ret < 0) {
				fprintf(stderr,"CLIENT: mnx_sendrec ret=%d\n", ret);
			}
			printf(MSG1_FORMAT, MSG1_FIELDS(m_ptr));
			free(m_ptr);
