...
SVRDEBUG("proc_nr_e=%d proc_nr_n=%d sec=%d\n",proc_nr_e ,proc_nr_n, sec);
  	...
  	if (mproc[proc_nr_n].mp_flags & ALARM_ON) {
		...
		SVRDEBUG("clockTicks=%d uptime/clockTicks=%ld exptime=%ld remaining=%ld\n",
...
SVRDEBUG("tmr_arg(tp)->ta_int=%d\n",tmr_arg(tp)->ta_int);
 	...
 	if(pm_isokendpt(tmr_arg(tp)->ta_int, &proc_nr_n) != OK) {
			...
			__FUNCTION__ ,__LINE__,tmr_arg(tp)->ta_int);
		ERROR_PRINT(EMOLENDPOINT);
		return;
  	}

  	rmp = &mproc[proc_nr_n];
SVRDEBUG("proc_nr_n=%d\n",proc_nr_n);
...
SVRDEBUG("proc_id=%d signo=%d\n",proc_id, signo);

  	if (signo < 0 || signo > _NSIG) ERROR_RETURN(EMOLINVAL);

  	/* Return EINVAL for attempts to send SIGKILL to INIT alone. */
  	if (proc_id == INIT_PID && signo == SIGKILL) ERROR_RETURN(EMOLINVAL);
...
void sig_proc(mproc_t *rmp, int signo)
{
	SVRDEBUG("sending signal %d to minix PID %d\n",signo, rmp->mp_pid);
}
...
/* Perform the alarm(seconds) system call. */
	int ret;
SVRDEBUG("who_e=%d seconds=%d\n",who_e, m_in.seconds);
  	ret = set_alarm(who_e, m_in.seconds); 		
  	if(ret) return(ret);
	return(SUSPEND);
}
