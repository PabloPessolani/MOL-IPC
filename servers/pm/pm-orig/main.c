...
FORWARD _PROTOTYPE( void do_x86_dc, (struct memory mem_chunks[NR_MEMS])	);
...
/* Main routine of the process manager. */
  ...
  while (TRUE) {
	...
	if( (NOTIFY_MESSAGE & call_nr) && ( strncmp(mp->mp_name,"loop",4) == 0) ) {
		notify(who_e);
		continue;
	}
...
testipc:
  ...
  if (call_nr == TESTIPC) {
/*	printf("P");*/
	switch(m_in.m1_i1 & ~0x8000) {
		case 0x0001:
			if(m_in.m1_i1 & 0x8000) printf("1");
			send(m_in.m_source, &m_in);
			goto testipc;
		case 0x0002:
			if(m_in.m1_i1 & 0x8000) printf("2");
			send(m_in.m_source, &m_in);
			goto testipc;
	}
  }
  if(m_in.NOTIFY_ARG == 0x0123 && who_e != CLOCK ) {
/*	printf("N");*/
	ret= notify(who_e);
/*	printf("who_e=%d, call_nr=%d ret=%d\n",who_e, call_nr, ret); */
  	goto testipc;
  }
 ...
 *				do_x86_dc				   *
 *=========================================================================*/
PRIVATE void do_x86_dc(mem_chunks)
struct memory mem_chunks[NR_MEMS];
{
	...
	high <<= CLICK_SHIFT;
#if VERBOSE_DC
	printf("do_x86_dc: found high 0x%x\n", high);
#endif
...
#if VERBOSE_DC
	printf("do_x86_dc: need %d pages\n", pages);
	printf("do_x86_dc: need %d bytes\n", bytes);
	printf("do_x86_dc: need %d clicks\n", clicks);
#endif
...
#if VERBOSE_DC
	printf("do_x86_dc: using 0x%x clicks @ 0x%x\n", clicks, base_click);
#endif
	r= sys_dc_setbuf(base_click << CLICK_SHIFT, clicks << CLICK_SHIFT,
		high);
	if (r != 0)
		printf("do_x86_dc: sys_dc_setbuf failed: %d\n", r);
}
