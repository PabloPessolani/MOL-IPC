...
{
	int vmid, rcode, result, proc_nr, ds_lpid, changed_nr;
	mnxsigset_t sigset;
	mproc_t *rmp;	
	proc_usr_t *rkp;	
	struct timespec t; 
	...
	vmid = atoi(argv[1]);
	if( vmid < 0 || vmid >= NR_DCS) ERROR_EXIT(EMOLRANGE);
...
void pm_init(int vmid)
{
	...
	pm_ep = mnx_replbind(vmid, pm_lpid, PM_PROC_NR);
	
	if (pm_ep != PM_PROC_NR) {
		if(pm_ep == EMOLSLOTUSED){
			rpm_ptr = &rpm;
			rcode = mnx_getprocinfo(vmid, PM_PROC_NR, rpm_ptr);
			...
			if( (TEST_BIT(rpm_ptr->p_rts_flags, BIT_REMOTE)) 
			 && (!TEST_BIT(rpm_ptr->p_misc_flags , MIS_BIT_RMTBACKUP))) {
				rcode = mnx_migr_start(vmid, PM_PROC_NR);
				if(rcode) ERROR_EXIT(rcode);
				rcode = mnx_migr_commit(pm_lpid, vmid, PM_PROC_NR, local_nodeid);
				if(rcode) ERROR_EXIT(rcode);
			}else{
  	...
  	while (TRUE) {
		...
		for (proc_nr=0, rmp=mproc; proc_nr < NR_PROCS; proc_nr++, rmp++) {
			...
			if ((rmp->mp_flags & (REPLYPENDING | ONSWAP | IN_USE | ZOMBIE)) ==
			   (REPLYPENDING | IN_USE)) {
				if ((s=mnx_send(rmp->mp_endpoint, &rmp->mp_reply)) != OK) {
