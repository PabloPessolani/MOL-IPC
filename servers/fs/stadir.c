...
/* Perform the chroot(name) system call. */
  ...
  if (!super_user) return(EMOLPERM); /* only su may chroot() */
...
/* Perform the readlink(name, buf) system call. */
  ...
  if (S_ISLNK(rip->i_mode) && (b = read_map(rip, (mnx_off_t) 0)) != NO_BLOCK) {
       if (m_in.name2_length <= 0) r = EMOLINVAL;
       else if (m_in.name2_length < rip->i_size) r = EMOLRANGE;
       else {
