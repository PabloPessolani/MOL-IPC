 ...
 */
  ...
  struct inode *rip, *xp;
	...
	// SVRDEBUG("free inode slot \n");  
  /* A free inode slot has been located.  Load the inode into it. */
	  xp->i_dev = dev;
	  xp->i_num = numb;
	  xp->i_count = 1;
	// SVRDEBUG(INODE_FORMAT1, INODE_FIELDS1(xp));
	// SVRDEBUG("despues de inode format \n");
	// SVRDEBUG("xp->i_dev= %d DEV_IMGRD=%d\n", xp->i_dev, DEV_IMGRD); 
	if (dev != NO_DEV) 
...
// SVRDEBUG("TEST antes de getSuper\n"); 
  /* Get the block where the inode resides. */
  sp = get_super(rip->i_dev);	/* get pointer to super block */
// SVRDEBUG("rip->i_dev= %d DEV_IMGRD=%d\n", rip->i_dev, DEV_IMGRD);   
// SVRDEBUG("TEST despues de getSuper\n"); 
...
//SVRDEBUG(INODE_FORMAT1, INODE_FIELDS1(NIL_INODE));
...
// SVRDEBUG("Paso truncate_inode y free_inode \n");
  } else {
    if (rip->i_pipe == I_PIPE) {/*SVRDEBUG("LLAMADA: truncate_inode(rip, 0)\n"); */truncate_inode(rip, 0);}
  }
  rip->i_pipe = NO_PIPE;  /* should always be cleared */
  if (rip->i_dirt == DIRTY) {/*SVRDEBUG("LLAMADA: rw_inode(rip, WRITING)\n")*/;rw_inode(rip, WRITING);}
  }
}
