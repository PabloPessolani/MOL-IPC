...
// int flags;     /* special flags, like O_NONBLOCK */
{
	...
	major = DEV2MAJOR(dev);
	minor = DEV2MINOR(dev);
	SVRDEBUG("op=%X dev=%d major=%d minor=%d proc_e=%d pos=%ld bytes=%d flags=%X\n", 
...
//}
	...
	m= &dev_mess;
	SVRDEBUG("dmap_driver=%d " MSG2_FORMAT, dp->dmap_driver, MSG2_FIELDS(m));
...
// SVRDEBUG("DESPUES dp->dmap_driver == NONE. \n");
	...
	if (dev_mess.REP_STATUS == SUSPEND) {
		if (flags & O_NONBLOCK) {
			...
			dev_mess.DEVICE = minor;
			(*dp->dmap_io)(dp->dmap_driver, &dev_mess);
			if (dev_mess.REP_STATUS == EMOLINTR) 
				dev_mess.REP_STATUS = EMOLAGAIN;
			dev_mess.REP_STATUS = EMOLNODEV;
		} else {
			/* Suspend user. */
 ...
 */
   ...
   * other process.
   */
	SVRDEBUG("task_nr=%d proc_e=%d " MSG2_FORMAT, task_nr, proc_e, MSG2_FIELDS(mess_ptr));
	r = mnx_sendrec(task_nr, mess_ptr);
	SVRDEBUG(MSG2_FORMAT, MSG2_FIELDS(mess_ptr));
