...
/* Perform the rename(name1, name2) system call. */
  ...
  if (r == OK) {
	...
	if (new_ip == NIL_INODE) {
		...
		if (old_ip->i_dev != old_dirp->i_dev) r = EMOLXDEV;
		if (odir && new_dirp->i_nlinks >=
		    (new_dirp->i_sp->s_version == V1 ? CHAR_MAX : SHRT_MAX) &&
		    !same_pdir && r == OK) r = EMOLMLINK;
	} else {
		if (old_ip == new_ip) r = SAME; /* old=new */

		/* has the old file or new file a file system mounted on it? */
		if (old_ip->i_dev != new_ip->i_dev) r = EMOLXDEV;

		ndir = ((new_ip->i_mode & I_TYPE) == I_DIRECTORY); /* dir ? */
		if (odir == TRUE && ndir == FALSE) r = EMOLNOTDIR;
		if (odir == FALSE && ndir == TRUE) r = EMOLISDIR;
	}
  }
  ...
  if (r == OK) {
	...
	numb = old_ip->i_num;		/* inode number of old file */
  	...
  	if (same_pdir) {
		r = search_dir(old_dirp, old_name, (mnx_ino_t *) 0, DELETE);
						/* shouldn't go wrong. */
		if (r==OK) (void) search_dir(old_dirp, new_name, &numb, ENTER);
	} else {
		r = search_dir(new_dirp, new_name, &numb, ENTER);
		if (r == OK)
		    (void) search_dir(old_dirp, old_name, (mnx_ino_t *) 0, DELETE);
	}
  }
...
//char dir_name[NAME_MAX];		/* name of directory to be removed */
{
  ...
  if (strcmp(dir_name, ".") == 0 || strcmp(dir_name, "..") == 0)return(EMOLINVAL);
