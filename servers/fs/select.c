...
{
	return (FD_ISSET(fid, &e->readfds) ? SEL_RD : 0) |
		(FD_ISSET(fid, &e->writefds) ? SEL_WR : 0) |
		(FD_ISSET(fid, &e->errorfds) ? SEL_ERR : 0);
}
...
{
	if ((ops & SEL_RD) && e->vir_readfds && FD_ISSET(fid, &e->readfds)
	        && !FD_ISSET(fid, &e->ready_readfds)) {
		FD_SET(fid, &e->ready_readfds);
		e->nreadyfds++;
	}
	if ((ops & SEL_WR) && e->vir_writefds && FD_ISSET(fid, &e->writefds) 
		&& !FD_ISSET(fid, &e->ready_writefds)) {
		FD_SET(fid, &e->ready_writefds);
		e->nreadyfds++;
	}
	if ((ops & SEL_ERR) && e->vir_errorfds && FD_ISSET(fid, &e->errorfds)
		&& !FD_ISSET(fid, &e->ready_errorfds)) {
		FD_SET(fid, &e->ready_errorfds);
		e->nreadyfds++;
	}
