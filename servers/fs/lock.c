...
/* Perform the advisory locking required by POSIX. */
 ...
 //  r = sys_datacopy(who_e, (vir_bytes) user_flock,
  ...
  if (r != OK) return(EMOLINVAL);

  /* Make some error checks. */
  ltype = flock.l_type;
  mo = f->filp_mode;
  if (ltype != F_UNLCK && ltype != F_RDLCK && ltype != F_WRLCK) return(EMOLINVAL);
  if (req == F_GETLK && ltype == F_UNLCK) return(EINVAL);
  if ( (f->filp_ino->i_mode & I_TYPE) != I_REGULAR) return(EMOLINVAL);
  if (req != F_GETLK && ltype == F_RDLCK && (mo & R_BIT) == 0) return(EMOLBADF);
  if (req != F_GETLK && ltype == F_WRLCK && (mo & W_BIT) == 0) return(EMOLBADF);
  ...
  if (last < first) return(EMOLINVAL);
  ...
  for (flp = &file_lock[0]; flp < & file_lock[NR_LOCKS]; flp++) {
	...
	if (nr_locks == NR_LOCKS) return(EMOLNOLCK);
  ...
  if (empty == (struct file_lock *) 0) return(EMOLNOLCK);	/* table full */
