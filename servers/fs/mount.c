...
"do_mount: s_imount = 0x%x (%x, %d), s_isup = 0x%x (%x, %d), fp_rootdir = 0x%x\n",
    ...
    xp->s_isup == fproc[FS_PROC_NR].fp_rootdir)
  {
     ...
     * is in use.
     */
    return(EMOLBUSY);  /* already mounted */
  }
  ...
  if (sp == NIL_SUPER) return(EMOLNFILE);  /* no super block available */
...
/* Unmount a file system by device number. */
  ...
  if (count > 1) return(EMOLBUSY); /* can't umount a busy file system */
