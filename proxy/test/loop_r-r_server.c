...
int child_function(int child) {
	...
	for( i = 0; i < loops; i++) {
		...
		m_ptr->m1_i1= i;
   		ret = mnx_reply(client_nr, (long) m_ptr);
		if(ret < 0) {
			fprintf(stderr,"CHILD %d: mnx_reply ret=%d\n", child, ret);
			exit(1);		
		}	
	}
...
int  main ( int argc, char *argv[] )
{
	...
	SVRDEBUG("MAIN SERVER: START synchronization from %d children: REQUEST\n", children);
	for( i = 0; i < children; i++){
	...
	/*--------------- Reply to  children -------------------------------*/
	SVRDEBUG("MAIN SERVER: START synchronization from %d children: REPLY\n", children);
	for( i = 0; i < children; i++){
	  	child_nr = SVR_NR + ((i+1) * 2);
	...
	/*--- Waiting STOP message from remote MAIN CLIENT----*/
 	SVRDEBUG("MAIN SERVER: Waiting STOP message from remote MAIN CLIENT\n");
	...
	SVRDEBUG("MAIN SERVER: STOP synchronization from %d children: REQUEST\n", children);
	for( i = 0; i < children; i++){
	  	child_nr = SVR_NR + ((i+1) * 2);
		do {
			ret = mnx_receive(child_nr, (long) m_ptr);
	...
	SVRDEBUG("MAIN SERVER: STOP synchronization from %d children: REPLY\n", children);
	for( i = 0; i < children; i++){
	  	child_nr = SVR_NR + ((i+1) * 2);
   		ret = mnx_send(child_nr,(long) m_ptr);
		if(ret < 0) {
