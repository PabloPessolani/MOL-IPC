...
void sender_proxy(void)
{
	...
	/* FIRST MESSAGE: CMD_COPYIN_DATA*/
		...
		ptr = buf_out.payload.pay_data;
		printf("%d PAYLOAD: %s ",pid, ptr); 

		ret = msgsnd(mq_snd, &buf_out, (sizeof(proxy_hdr_t) + buf_out.header.c_vcopy.v_bytes), 0);
...
void  main ( int argc, char *argv[] )
{
	...
	/*REPLY WITH CMD_COPYIN_ACK */
		/* BUILD THE REPLY HEADER */
		buf_out.header.c_cmd 	= CMD_COPYIN_ACK;
		...
		buf_out.header.c_len 	= 0;
		buf_out.header.c_rcode	= OK;

		buf_out.header.c_vcopy.v_saddr = h_ptr->c_vcopy.v_daddr;
		buf_out.header.c_vcopy.v_daddr = h_ptr->c_vcopy.v_saddr;
		buf_out.header.c_vcopy.v_bytes = h_ptr->c_vcopy.v_bytes;

		ret = msgsnd(mq_rcv, &buf_out, sizeof(proxy_hdr_t), 0); 
