...
void receiver_proxy(void)
{
	...
	strcpy(&payload.pay_data,"ESTOS SON DATOS ENVIADOS DESDE REMOTO A LOCAL");
	/* send a message - build the header */
	h_ptr = &header;
	h_ptr->hdr_type	= CMD_COPYOUT_DATA;
	h_ptr->hdr_src	= rmt_ep;
	h_ptr->hdr_snode = rmt_nodeid;
	h_ptr->hdr_dnode = lcl_nodeid;
	h_ptr->hdr_dcid	= dcid;
	h_ptr->hdr_dst	= lcl_ep;
	h_ptr->hdr_len = strlen(&payload.pay_data);
	h_ptr->hdr_dataaddr = 0x7890;
	h_ptr->hdr_rcode = OK;
printf("%d "HDR_FORMAT,pid, HDR_FIELDS(h_ptr)); 

	p_ptr  		= &payload.pay_data;
printf("%d DATA[%s]\n",pid, p_ptr);
...
void  main ( int argc, char *argv[] )
{
	...
	strcpy(buffer,"DATOS LOCALES");	
	printf("ANTES [%s]\n",buffer);
	ret = mnx_vcopy(rmt_ep, 0x1234, lcl_ep, buffer,  strlen(buffer));	
	if( ret != 0 )
	   	printf("VCOPY ret=%d\n",ret);
	printf("RECIBIDO [%s]\n",buffer);
	sleep(5);
