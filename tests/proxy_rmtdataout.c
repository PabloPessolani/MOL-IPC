...
char buffer[] = "012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789";
...
void sender_proxy(void)
{
	...
	/* FIRST MESSAGE: CMD_COPYOUT_RQST */
		...
		printf("%d " VCOPY_FORMAT,pid, VCOPY_FIELDS(h_ptr));

		ret = msgsnd(mq_snd, &buf_out, sizeof(proxy_hdr_t), 0);
...
void receiver_proxy(void)
{
	...
	/*FIRST REPLY: CMD_COPYOUT_DATA */
		...
		ptr = buf_in.payload.pay_data;
		printf("%d PAYLOAD: %s ",pid, ptr); 
...
void  main ( int argc, char *argv[] )
{
	...
	/*REPLY WITH CMD_COPYOUT_DATA */
		/* BUILD THE REPLY HEADER */
		buf_out.header.c_cmd 	= CMD_COPYOUT_DATA;
		...
		buf_out.header.c_len 	= (sizeof(proxy_hdr_t) + h_ptr->c_vcopy.v_bytes);
		buf_out.header.c_rcode	= OK;

		memcpy(&buf_out.header.c_vcopy, &buf_in.header.c_vcopy, sizeof(vcopy_t));
		strncpy(&buf_out.payload.pay_data, buffer, h_ptr->c_vcopy.v_bytes);

		ret = msgsnd(mq_rcv, &buf_out, (sizeof(proxy_hdr_t) + h_ptr->c_vcopy.v_bytes), 0); 
