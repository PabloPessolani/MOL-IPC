...
static int mfs_open(const char *path, struct fuse_file_info *fi)
{
	...
	fd = MOL_open((char*) fpath, fi->flags);
    if (fd < 0)
...
static int mfs_getattr(const char *path, struct stat *stbuf)
{
	...
	MOL_Bind();
	res = MOL_lstat((char*) fpath, stbuf);
    if (res != 0)
		       ...
		       off_t offset, struct fuse_file_info *fi)
{
	...
	while ((de = MOL_readdir(dp)) != NULL) {
		...
		if (MOL_stat((char*) fpath, &st) < 0) return nil;
		// if (!S_ISDIR(st.st_mode)) { errno= ENOTDIR; return nil; }
...
int mfs_link(const char *path, const char *newpath)
{
	...
	MOL_Bind();        
    retstat = MOL_link(fpath, fnewpath);
    if (retstat < 0)
...
int mfs_rename(const char *path, const char *newpath)
{
	...
	MOL_Bind();      
    retstat = MOL_rename(fpath, fnewpath);
    if (retstat < 0)
...
int mfs_readlink(const char *path, char *link, size_t size)
{
	...
	MOL_Bind();      
    retstat = MOL_readlink(fpath, link, size - 1);
    if (retstat < 0)
...
int mfs_symlink(const char *path, const char *link)
{
	...
	MOL_Bind();          
    retstat = MOL_symlink(path, flink);
    if (retstat < 0)
...
static int mfs_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
	...
	MOL_Bind();
	fd = MOL_open((char*) fpath, O_RDONLY);
	if (fd == -1)
		return -errno;
		     ...
		     off_t offset, struct fuse_file_info *fi)
{
	...
	MOL_Bind();
	fd = MOL_open((char*) fpath, O_WRONLY);
	if (fd == -1)
		return -errno;
...
static int mfs_utime(const char *path, struct utimbuf *ubuf)
{
	...
	MOL_Bind();
	retstat = MOL_utime((char*) fpath, ubuf);
    if (retstat < 0)
...
static int mfs_chown(const char *path, uid_t uid, gid_t gid)
{
	...
	MOL_Bind();
	retstat = MOL_chown((char*) fpath, uid, gid);
    if (retstat < 0)
...
static int mfs_chmod(const char *path, mode_t mode)
{
	...
	MOL_Bind();
	retstat = MOL_chmod((char*) fpath, mode);
    if (retstat < 0)
...
static int mfs_truncate(const char *path, off_t length)
{
	...
	MOL_Bind();
	retstat = MOL_truncate((char*) fpath, length);
	if (retstat < 0)
		return -errno;
...
int mfs_mknod(const char *path, mode_t mode, dev_t dev)
{
    ...
    //  is more portable
    if (S_ISREG(mode)) {
        retstat = MOL_open(fpath, O_CREAT | O_EXCL | O_WRONLY);
	if (retstat < 0)
	...
	if (S_ISFIFO(mode)) {
	    retstat = MOL_mknod(fpath, mode | S_IFIFO, (dev_t) 0);
	    if (retstat < 0)
		retstat = mfs_error("mfs_mknod -> mkfifo");
	} else {
	    retstat = MOL_mknod(fpath, mode, dev);
	    if (retstat < 0)
...
//   .ftruncate = bb_ftruncate, //Poner PH por ahora
//   .fgetattr = bb_fgetattr //Lo mismo aca
// };
