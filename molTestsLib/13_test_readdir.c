...
//****************************** MODIFICADO PAP :  START ****************************
	if ( argc != 2 && argc != 3)	{
		fprintf(stderr, "Usage: %s <MolNameToRead> [buf_size] \n", argv[0] );
		exit(1);
	}
...
//****************************** MODIFICADO PAP :  END  ****************************
	...
	// /*----------------------OPEN DIR-------------------*/
	// dp = mol_opendir(argv[3]);
	// SVRDEBUG("mol_opendir reply: " DIR_FORMAT, DIR_FIELDS(dp));
	
	// /*----------------------READ DIR-------------------*/	
	// de = mol_readdir(dp);
	// SVRDEBUG("mol_readdir reply: " DIRENT_FORMAT, DIRENT_FIELDS(de));
