...
void  main( int argc, char *argv[])
{
	...
	//Para CLOSEDIR
	int resp_cd;
	//Para TRUNCATE y FTRUNCATE
	int bytesToTruncate = 0;		

	if ( argc != 5 && argc != 6)	{
		fprintf(stderr, "Usage: %s <dcid> <clt_nr> <MolFilename> <bytesToTruncate> [buf_size] \n", argv[0] );
		exit(1);
	}
	...
	bytesToTruncate = atoi(argv[4]);

	INFODEBUG("CLIENT bytesToTruncate=%d\n", bytesToTruncate);
	...
	/*----------------------TRUNCATE-------------------*/
	fd = MOL_truncate(argv[3], bytesToTruncate);	//Para test de truncate
		
	if( fd < 0 ) {
		fprintf(stderr,"MOL_truncate fd=%d\n",fd);
		exit(1);
	}
	INFODEBUG("MOL_truncate reply is=%d\n",fd);
