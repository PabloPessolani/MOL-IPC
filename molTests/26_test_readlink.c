...
void  main( int argc, char *argv[])
{
	...
	if ( argc != 5 && argc != 6)	{
		fprintf(stderr, "Usage: %s <dcid> <clt_nr> <MolFilename> <bytesToRead> [buf_size] \n", argv[0] );
		exit(1);
	}
	...
	bytesToRead = atoi(argv[4]);

	INFODEBUG("CLIENT bytesToRead=%d\n", bytesToRead);
	...
	/*----------------------READLINK-------------------*/
	fd = MOL_readlink(argv[3], (void *) buffer, bytesToRead);	//Para test de readlink
		
	if( fd < 0 ) {
		fprintf(stderr,"MOL_readlink fd=%d\n",fd);
		exit(1);
	}
	INFODEBUG("MOL_readlink reply is=%d\n",fd);
