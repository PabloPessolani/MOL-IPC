...
void  main( int argc, char *argv[])
{
	...
	if ( argc != 4 && argc != 5)	{
		fprintf(stderr, "Usage: %s <dcid> <clt_nr> <MolLinkName> [buf_size] \n", argv[0] );
		exit(1);
	}
	...
	/*----------------------UNLINK-------------------*/
	fd = MOL_unlink(argv[3]);	//Para test de unlink
		
	if( fd < 0 ) {
		fprintf(stderr,"MOL_unlink fd=%d\n",fd);
		exit(1);
	}
	INFODEBUG("MOL_unlink reply is=%d\n",fd);
