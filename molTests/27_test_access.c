...
void  main( int argc, char *argv[])
{
	...
	if ( argc != 5 && argc != 6)	{
		fprintf(stderr, "Usage: %s <dcid> <clt_nr> <MolFilename> <access> [buf_size] \n", argv[0] );
		exit(1);
	}
	...
	accessToTest = atoi(argv[4]);

	// #define R_OK    4/* test for read permission */
	// #define W_OK    2/* test for write permission */
	// #define X_OK    1/* test for execute (search) permission */
	// #define F_OK    0/* test for presence of file */
	INFODEBUG("CLIENT accessToTest=%d,  accessToTest(Hex)=%X\n", accessToTest, accessToTest);
	...
	/*----------------------ACCESS-------------------*/
	fd = MOL_access(argv[3], accessToTest);	//Para test de access
		
	if( fd < 0 ) {
		fprintf(stderr,"MOL_access fd=%d\n",fd);
		exit(1);
	}
	INFODEBUG("MOL_access reply is=%d\n",fd);
