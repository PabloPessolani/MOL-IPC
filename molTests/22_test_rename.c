...
void  main( int argc, char *argv[])
{
	...
	if ( argc != 5 && argc != 6)	{
		fprintf(stderr, "Usage: %s <dcid> <clt_nr> <oldMOLFilename> <newMOLFilename> [buf_size] \n", argv[0] );
		exit(1);
	}
	...
	/*----------------------RENAME-------------------*/
	fd = MOL_rename(argv[3], argv[4]);	//Para test de rename
		
	if( fd < 0 ) {
		fprintf(stderr,"MOL_rename fd=%d\n",fd);
		exit(1);
	}
	INFODEBUG("MOL_rename reply is=%d\n",fd);
