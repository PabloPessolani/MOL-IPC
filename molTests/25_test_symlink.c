...
void  main( int argc, char *argv[])
{
	...
	if ( argc != 5 && argc != 6)	{
		fprintf(stderr, "Usage: %s <dcid> <clt_nr> <MOLFileToLink> <linkToMOLFile> [buf_size] \n", argv[0] );
		exit(1);
	}
	...
	/*----------------------SYMLINK-------------------*/
	fd = MOL_symlink(argv[3], argv[4]);	//Para test de symlink
		
	if( fd < 0 ) {
		fprintf(stderr,"MOL_link fd=%d\n",fd);
		exit(1);
	}
	INFODEBUG("MOL_link reply is=%d\n",fd);
