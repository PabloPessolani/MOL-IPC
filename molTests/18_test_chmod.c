...
void  main( int argc, char *argv[])
{
	...
	int retChmod;
	char *mode;
	mnx_mode_t modoNuevo;

	if ( argc != 5 && argc != 6)	{
		fprintf(stderr, "Usage: %s <dcid> <clt_nr> <MolFilename> <mode> [buf_size] \n", argv[0] );
		exit(1);
	}
	...
	modoNuevo = parseMode(mode);
	INFODEBUG("MODE buf_size=%o\n", modoNuevo);
...
// #ifdef ANULADO
	...
	/*-----------CHMOD wiht MOL_chmod ---------*/
	retChmod = MOL_chmod(argv[3], modoNuevo);

	if ( retChmod < 0 ) {
		fprintf(stderr, "MOL_chmod=%d\n", retChmod);
		exit(1);
	}
	TESTDEBUG("MOL_chmod results are=%d\n", retChmod);
