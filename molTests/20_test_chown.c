...
ast:eY1NhncLIuvTQ:0:0:Andrew S. Tanenbaum:/home/ast:
...
#define USER_ID   0 /* root:##root:0:0:Big Brother:/root: */
#define USER_ID_TO_CHANGE  9 /* www:*:9:9:World Wide Web:/usr/www: */
#define GROUP_ID   0 /*operator:*:0:*/
#define GROUP_ID_TO_CHANGE  3 /*other:*:3:*/
...
void  main( int argc, char *argv[])
{
	...
	if ( argc != 5 && argc != 6)	{
		fprintf(stderr, "Usage: %s <dcid> <clt_nr> <MolFilename> <root> [buf_size] \n", argv[0] );
		exit(1);
	}
	...
	/*si este parametro esta seteado es para cambiar el archivo a Root*/
	becomeRoot = atoi(argv[4]);
	...
	INFODEBUG("BIND CLIENT dcid=%d clt_pid=%d clt_nr=%d clt_ep=%d\n",
	          dcid,
	          clt_pid,
	          clt_nr,
	          clt_ep);
	...
	fd = MOL_open(argv[3], O_RDONLY | O_NONBLOCK);	//Para test de opendir
	...
	/*-----------CHOWN wiht MOL_chown ---------*/
	if (becomeRoot)
		retChown = MOL_chown(argv[3], USER_ID, GROUP_ID);
	else
		retChown = MOL_chown(argv[3], USER_ID_TO_CHANGE, GROUP_ID_TO_CHANGE);

	if ( retChown < 0 ) {
		fprintf(stderr, "MOL_chown=%d\n", retChown);
		exit(1);
	}
	TESTDEBUG("MOL_chown results are=%d\n", retChown);
