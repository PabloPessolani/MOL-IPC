...
main(int argc, char *argv[])
{
	...
	do
	{
		...
		ip_fd= mol_open(device_s, O_RDWR);
		CMDDEBUG("ip_fd=%d\n", ip_fd);
		...
		if (v_flag) {
			if (!get_ipconf(ip_fd, &ipconf)) {
				...
				printf("%s:%s: address %s", prog_name, device_s,
...
static void set_hostaddr (int ip_fd, char *hostaddr_s, int ins)
{
	mnx_ipaddr_t ipaddr;
	struct nwio_ipconf ipconf;
	int result;

	CMDDEBUG("ip_fd=%d hostaddr_s=%s ins=%d\n", ip_fd, hostaddr_s, ins);

	ipaddr= mnx_inet_addr(hostaddr_s);
	if (ipaddr == (mnx_ipaddr_t)(-1))
	{
	...
	result= mol_ioctl(ip_fd, NWIOSIPCONF, &ipconf);
	CMDDEBUG("result=%d sizeof(ipconf)=%d\n", result,sizeof(ipconf));
...
static int check_ipaddrset (int ip_fd)
{
	struct nwio_ipconf ipconf;
...
static int get_ipconf (int ip_fd, struct nwio_ipconf *ref_ipaddr)
{
	int flags;
	int error, result;
	nwio_ipconf_t ipconf, *ipc_ptr;

	CMDDEBUG("ip_fd=%d\n", ip_fd);
	
	flags= mol_fcntl(ip_fd, F_GETFL);

	CMDDEBUG("flags=%X NWIOGIPCONF=%X\n", flags, NWIOGIPCONF);
	
	mol_fcntl(ip_fd, F_SETFL, flags | O_NONBLOCK);
	
	result= mol_ioctl(ip_fd, NWIOGIPCONF, &ipconf);
	error= errno;
	CMDDEBUG("result=%d error=%d\n", result, error);

	mol_fcntl(ip_fd, F_SETFL, flags);

	if (result <0 && error != EMOLAGAIN)
	{
		errno= error;
	...
	{
		ipc_ptr = &ipconf;
		CMDDEBUG(IPCONF_FORMAT, IPCONF_FIELDS(ipc_ptr));
		*ref_ipaddr = ipconf;
	}
	return result>=0;
}
...
static void set_netmask (int ip_fd, char *netmask_s,int ins)
{
	mnx_ipaddr_t netmask;
	struct nwio_ipconf ipconf;
	int result;

	netmask= mnx_inet_addr(netmask_s);
	if (netmask == (mnx_ipaddr_t)(-1))
	{
...
static void set_mtu (int ip_fd, char *mtu_s)
{
	mnx_ipaddr_t netmask;
	int result;
	long mtu;
...
static int check_netmaskset (int ip_fd)
{
	struct nwio_ipconf ipconf;
