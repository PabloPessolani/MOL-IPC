...
printf("local_nodeid=%d\n",local_nodeid);
	...
	if( (src_pid = fork()) != 0 )	{		/* FIRST CHILD SOURCE */
		if( (dst_pid = fork()) != 0 )	{	/* PARENT  REQUESTER */
			...
			/* TWO RECEIVES to block SENDER and RECEIVER */		
    			ret = mnx_receive(ANY, (long) &m);
   			printf("REQUESTER 1st RECEIVE " MSG5_FORMAT, MSG5_FIELDS(m_ptr));
	    		ret = mnx_receive(ANY, (long) &m);
   			printf("REQUESTER 2nd RECEIVE " MSG5_FORMAT, MSG5_FIELDS(m_ptr));
