...
void usage(char* errmsg, ...) {
	...
	fprintf(stderr, "Usage: rdisk <config_file> <replicate_flag>\n");
...
int main (int argc, char *argv[] )
{
	...
	if ( argc != 3) { /* config file - replicate=1*/
	// if ( argc != 2) { /* config file - replicate=1*/
	...
	c_file = argv[1];
	replicate_flag = atoi(argv[2]);
	
	TASKDEBUG("replicate_flag(VEER)=%d\n",replicate_flag);
	...
	while (f_dev < NR_DEVS){
		for( i = f_dev; i < NR_DEVS; i++){
			...
			TASKDEBUG("i=%d stat0 %s rcode=%d\n", i, devvec[i].img_ptr, rcode);
			if(rcode){
			...
			for( j = l_dev; j < NR_DEVS; j++){
				...
				TASKDEBUG("j=%d stat1 %s rcode=%d\n",j , devvec[i].img_ptr, rcode);
				if(rcode){
					fprintf( stderr,"\nERROR %d: Device %s minor_number %d is not valid\n", rcode, c_file, j );
					l_dev++;
				}
	...
	while (nr_req > 0) { /*2*/
		...
		if ((opcode == DEV_GATHER) ||(opcode == DEV_CGATHER))  {			/* copy data */ /*DEV_GATHER read from an array (com.h)*/
			...
			do	{
				...
				if(bytes < 0) ERROR_EXIT(-errno); // PAP 
...
int rd_init(void )
{
	...
	rcode = mnx_getprocinfo(VMID, RDISK_PROC_NR, &proc_rd);
	if(rcode) ERROR_EXIT(rcode);
	rd_ptr = &proc_rd;
	...
	if( replicate_flag != DO_REPLICATE) { // WITHOUT REPLICATION 
		if( TEST_BIT(rd_ptr->p_rts_flags, BIT_SLOT_FREE)) {
			...
			rd_ep = mnx_bind(VMID,RDISK_PROC_NR);
			if(rd_ep < 0 ) ERROR_EXIT(rd_ep);			
		...
		if( TEST_BIT(rd_ptr->p_rts_flags, BIT_SLOT_FREE)) { // PRIMARY as REPLICA 
			...
			rd_ep = mnx_replbind(VMID,rd_lpid,RDISK_PROC_NR);
			if(rd_ep < 0 ) ERROR_EXIT(rd_ep);
			rcode = sys_bindproc(rd_ep, rd_lpid, REPLICA_BIND);
			if(rcode < 0) ERROR_EXIT(rcode)			
		}else{											// SECONDARY as BACKUP 
			TASKDEBUG("Starting RDISK BACKUP\n");
			rd_ep = mnx_bkupbind(VMID,rd_lpid,RDISK_PROC_NR, rd_ptr->p_nodeid);
			if(rd_ep < 0 ) ERROR_EXIT(rd_ep);
		//	rcode = sys_bindproc(rd_ep, rd_lpid, BKUP_BIND);
		//	if(rcode < 0) ERROR_EXIT(rcode)				
		}
	}
	rcode = mnx_getprocinfo(VMID, RDISK_PROC_NR, &proc_rd);
