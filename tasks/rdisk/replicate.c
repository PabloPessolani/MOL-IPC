...
int init_replicate(void)
{
	...
	*--------------------------------------------------------------------------------------*/
	sprintf( replica_name, "RDISK%02d", dc_ptr->dc_dcid);
	TASKDEBUG("replica_name=%s\n", replica_name);
	sprintf( Spread_name, "4803");
	sprintf( User, "RDISK%0d.%0d", local_nodeid, dc_ptr->dc_dcid);
	TASKDEBUG("User=%s\n", User);
...
//					TASKDEBUG("grp id is %d %d %d\n",memb_info.gid.id[0], memb_info.gid.id[1], memb_info.gid.id[2] );
		...
		if( Is_caused_join_mess( service_type ) )	{
			...
			*----------------------------------------------------------------------------------------------------*/
			...
			// if( nr_nodes < 1 || nr_nodes <= dc_ptr->dc_nr_nodes){
						
			// if (local_nodeid != primary_mbr){/*SLAVE*/
                // // creo el thread_slave (hago el bind y desde ahí:
                // // send, m3ipc= DEV_CFULL:solicitando establecer una conexión para poder sincronizar 
                // // cuando tengo el OK
                // // hago la actualización hasta terminar) 
			...
			// else{/*MASTER*/
				...
				// rcode = pthread_create( &mastercopy_thread, NULL, mastercopy_main, 0 );
				// if( rcode)ERROR_EXIT(rcode);
