...
pthread_t eth_rx_thread[EC_PORT_NR_MAX]; 	
pthread_t eth_tx_thread[EC_PORT_NR_MAX];
pthread_t eth_inet_thread;
...
//		= virt_to_bus(ei_ptr->tbuf[tx_slot_nr]) & 0xffffff;
	...
	MTX_LOCK(send_mutex[ec_ptr->ec_port]);
	COND_SIGNAL(send_barrier[ec_ptr->ec_port]);
	COND_WAIT(smain_barrier[ec_ptr->ec_port],send_mutex[ec_ptr->ec_port]);
	// HERE REPORT TO INET 
	ec_ptr->flags = ECF_PACK_SEND;
    reply(ec_ptr, OK, TRUE);
	MTX_UNLOCK(send_mutex[ec_ptr->ec_port]);
...
static void send_thread(void *arg)
{
	...
	while(1) {
		...
		COND_WAIT(send_barrier[ec_ptr->ec_port],send_mutex[ec_ptr->ec_port]);
...
static void  eth_init(void)
{
	...
	do { 
		...
		TASKDEBUG("ETH: mnx_wait4bind_T  rcode=%d\n", rcode);
		if (rcode == EMOLTIMEDOUT) {
			TASKDEBUG("ETH: mnx_wait4bind_T TIMEOUT\n");
			continue ;
		}else if( rcode < 0) 
...
#define 	RECEIVE_TIMEOUT	5000
	...
	while(TRUE){
		...
		COND_WAIT(eth_barrier, main_mutex);// wait until all operations end  
		MTX_UNLOCK(main_mutex);
	}
}
...
static void main_init()
{
	...
	rcode = pthread_cond_init(&eth_barrier, NULL);
	if(rcode) ERROR_EXIT(rcode);
