/*
	...
	It reads the complete image file and store it into a local file named
		loop_readimg.img 
	To check correctness and integrity you must do
		diff <(od -An -tx1 -w1 loop_readimg.img) <(od -An -tx1 -w1 image_file.img)
	or 
...
void  main( int argc, char *argv[])
{
	...
	do { 
		m =  m1; 
		m.POSITION = position;
		TESTDEBUG("DEV_READ REQUEST: loops=%d " MSG2_FORMAT, loops, MSG2_FIELDS(m_ptr) );
		rcode = mnx_sendrec(svr_ep, (long) &m); 
		if( rcode != 0 ){
			fprintf(stderr,"DEV_READ mnx_sendrec rcode=%d\n",rcode);
			exit(1);
		}
		TESTDEBUG("DEV_READ REPLY: " MSG2_FORMAT,MSG2_FIELDS(m_ptr) );
		if( m.REP_STATUS < 0) {
			fprintf(stderr,"DEV_READ REPLY ERROR REP_STATUS=%d\n",m.REP_STATUS);
			exit(1);
		}
		bytes= fwrite( (void *) buffer, 1, m.REP_STATUS, fp_img);
		if( bytes < 0) {
			fprintf(stderr,"DEV_READ fwrite ERROR rcode=%d\n",bytes);
			exit(1);
		}
		position += m.REP_STATUS;
		loops++;
	}while( m.REP_STATUS != 0); /* means EOF */
     	t_stop  = dwalltime();
	TESTDEBUG("DEV_READ total bytes read=%d", position);
