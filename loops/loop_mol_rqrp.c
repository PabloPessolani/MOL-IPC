...
void  main ( int argc, char *argv[] )
{
	...
	if( (src_pid = fork()) != 0 )	{		/* PARENT = DESTINATION */
		...
		do {
   		...
   		printf("RECEIVER: src_pid=%d has binded on endpoint=%d\n", src_pid, src_ep);
...
*/
		...
		for( i = 0; i < loops; i++) {
			ret = mnx_rcvrqst((long) &m);
	    		ret = mnx_reply(src_ep, (long) &m);
		}

     	t_stop  = dwalltime();

   		printf("RCVRQST msg: m_source=%d, m_type=%d, m1_i1=%d, m1_i2=%d, m1_i3=%d\n",
			m.m_source,
			m.m_type,
			m.m1_i1,
			m.m1_i2,
		...
		t_total = (t_stop-t_start);
 		...
 		printf("Loops = %d\n", loops);
 		printf("Time for SENDREC/(RCVRQST:REPLY)= %f[ms]\n", 1000*t_total/(double)loops);
