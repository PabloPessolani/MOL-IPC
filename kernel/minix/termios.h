...
#ifndef _MNX_TERMIOS_H
#define _MNX_TERMIOS_H

typedef unsigned short mnx_tcflag_t;
typedef unsigned char mnx_cc_t;
typedef unsigned int mnx_speed_t;

#define NCCS		   20	/* size of cc_c array, some extra space
				 * for extensions. */

/* Primary terminal control structure. POSIX Table 7-1. */
typedef struct mnx_termios_s {
  mnx_tcflag_t c_iflag;		/* input modes */
  mnx_tcflag_t c_oflag;		/* output modes */
  mnx_tcflag_t c_cflag;		/* control modes */
  mnx_tcflag_t c_lflag;		/* local modes */
  mnx_speed_t  c_ispeed;		/* input speed */
  mnx_speed_t  c_ospeed;		/* output speed */
  mnx_cc_t c_cc[NCCS];		/* control characters */
} mnx_termios_t;
...
_PROTOTYPE( mnx_speed_t mnx_cfgetispeed, (const struct mnx_termios_s *_termios_p)	     );
_PROTOTYPE( mnx_speed_t mnx_cfgetospeed, (const struct mnx_termios_s *_termios_p)	     );
_PROTOTYPE( int mnx_cfsetispeed, (struct mnx_termios_s *_termios_p, mnx_speed_t _speed)    );
_PROTOTYPE( int mnx_cfsetospeed, (struct mnx_termios_s *_termios_p, mnx_speed_t _speed)    );
_PROTOTYPE( int tcgetattr, (int _filedes, struct mnx_termios_s *_termios_p)        );
_PROTOTYPE( int tcsetattr, \
	(int _filedes, int _opt_actions, const struct mnx_termios_s *_termios_p)   );
#endif // ANULADO

#ifdef _MINIX
#define mnx_cfgetispeed(termios_p)		((termios_p)->c_ispeed)
#define mnx_cfgetospeed(termios_p)		((termios_p)->c_ospeed)
#define mnx_cfsetispeed(termios_p, speed)	((termios_p)->c_ispeed = (speed), 0)
#define mnx_cfsetospeed(termios_p, speed)	((termios_p)->c_ospeed = (speed), 0)
