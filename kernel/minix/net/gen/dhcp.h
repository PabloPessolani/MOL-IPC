...
typedef struct dhcp {
	...
	u16_t		flags;		/* Flags. */
	mnx_ipaddr_t	ciaddr;		/* Client IP address. */
	mnx_ipaddr_t	yiaddr;		/* "Your" IP address. */
	mnx_ipaddr_t	siaddr;		/* Boot server IP address. */
	mnx_ipaddr_t	giaddr;		/* Relay agent (gateway) IP address. */
...
int dhcp_settag(dhcp_t *_dp, int _tag, void *_data, mnx_size_t _len);
int dhcp_gettag(dhcp_t *_dp, int _searchtag, u8_t **_pdata, mnx_size_t *_plen);
