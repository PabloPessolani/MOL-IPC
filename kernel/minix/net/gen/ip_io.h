...
{
	u32_t	nwic_flags;
	mnx_ipaddr_t nwic_ipaddr;
	mnx_ipaddr_t nwic_netmask;
} nwio_ipconf2_t;
...
#define IPCONF_FORMAT "nwic_flags=%X nwic_ipaddr=%X nwic_netmask=%X nwic_mtu=%d\n" 
#define IPCONF_FIELDS(p) p->nwic_flags, p->nwic_ipaddr, p->nwic_netmask, p->nwic_mtu
...
#define IPOPT_FORMAT "nwio_flags=%X nwio_tos=%d nwio_ttl=%d nwio_df=%d nwio_proto=%X\n" 
#define IPOPT_FIELDS(p) p->nwio_flags, p->nwio_tos, p->nwio_ttl, p->nwio_df, p->nwio_proto
