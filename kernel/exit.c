...
atomic_t local_nodeid = ATOMIC_INIT(DVS_NO_INIT); /* local_nodeid = DVS_NO_INIT = (-1) means that the DVS is unitialized! */
...
//{
//	printk(KERN_ALERT "M3-IPC: exit_unbind local_nodeid:%d\n",atomic_read(&local_nodeid));
//	mol_exit_unbind(code);
//}
...
//	if(!DVS_NOT_INIT()) {
	...
	if( atomic_read(&local_nodeid) != DVS_NO_INIT ) {
		// printk(KERN_ALERT "do_exit: local_nodeid:%d\n",atomic_read(&local_nodeid));
		exit_unbind(code);
	}
//	else{
//		printk(KERN_ALERT "M3-IPC: local_nodeid:%d\n",atomic_read(&local_nodeid));
//	}
	exit_irq_thread();
