...
//****************************** MODIFICADO PAP :  END  ****************************
	...
	/*----------------------FILE_SEEK-------------------*/
	seek_res = mol_lseek(fd, 15, SEEK_SET);
		
	if( seek_res < 0 ) {
		fprintf(stderr,"mol_lseek =%d\n",seek_res);
		exit(1);
	}
	TESTDEBUG("mol_lseek result is=%d\n",seek_res);		

	/*----------------------FILE_READ-------------------*/
	bytes = mol_read(fd, (void *) buffer, 15); 
	if( bytes < 0) {
		fprintf(stderr," mol_read ERROR rcode=%d\n",bytes);
		exit(1);
	}
	...
	TESTDEBUG("Buffer cleared is=%s\n", buffer);

	/*----------------------FILE_SEEK-------------------*/
	seek_res = mol_lseek(fd, -20, SEEK_END);
	...
	/*----------------------FILE_READ-------------------*/
	bytes = mol_read(fd, (void *) buffer, 20); 
	if( bytes < 0) {
		fprintf(stderr," mol_read ERROR rcode=%d\n",bytes);
		exit(1);
	}
