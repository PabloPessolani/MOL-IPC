...
//****************************** MODIFICADO PAP :  START ****************************
	if ( argc != 3 && argc != 4)	{
		fprintf(stderr, "Usage: %s <MOLFilename> <bytesToRead> [buf_size] \n", argv[0] );
		exit(1);
	}
...
//****************************** MODIFICADO PAP :  END  ****************************		
	...
	TESTDEBUG("CLIENT bytesToRead=%d\n", bytesToRead);
	...
	/*----------------------READLINK-------------------*/
	fd = mol_readlink(argv[1], (void *) buffer, bytesToRead);	//Para test de readlink
		
	if( fd < 0 ) {
		fprintf(stderr,"mol_readlink fd=%d\n",fd);
		exit(1);
	}
	TESTDEBUG("mol_readlink reply is=%d\n",fd);
