...
//****************************** MODIFICADO PAP :  START ****************************
	if ( argc != 4 )	{
		fprintf(stderr, "Usage: %s <MolFilename> <mode> <dev> \n", argv[0] );
		exit(1);
	}

	mode = (int) strtol(argv[2], &endptr, 8);
	dev = (int) strtol(argv[3], &endptr, 16);

	init_m3ipc();

	SVRDEBUG("(%d)Starting %s in dcid=%d endpoint=%d MolFilename=%s mode=%o dev=%X\n",
	         clt_lpid, argv[0], dc_ptr->dc_dcid, clt_ep, argv[1],argv[2],argv[3]);

//****************************** MODIFICADO PAP :  END  ****************************
		   
	SVRDEBUG("CLIENT pause before SENDREC\n");
	sleep(2); 

	/*----------------------mol_MKNOD-------------------*/
	rcode = mol_mknod(argv[1], mode, dev);
	if( rcode != 0 ) {
		fprintf(stderr,"mol_MKNOD rcode=%d\n",rcode);
		exit(1);
	}

	SVRDEBUG("mol_MKNOD resuts=%d\n",rcode);	
