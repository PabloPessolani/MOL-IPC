...
//****************************** MODIFICADO PAP :  START ****************************
	if ( argc != 2 && argc != 3)	{
		fprintf(stderr, "Usage: %s <MolNameToCreat> [buf_size] \n", argv[0] );
		exit(1);
	}
...
//****************************** MODIFICADO PAP :  END  ****************************
	...
	/*----------------------MKDIR-------------------*/
	fd = mol_mkdir(argv[1], O_RDWR | O_NONBLOCK);	//Para test de mkdir
	SVRDEBUG("mol_mkdir result is=%d\n",fd);

	if( fd < 0 ) {
		//fprintf(stderr,"mol_mkdir fd=%d\n",fd);
		ERROR_RETURN (errno);
		exit(1);
	}
	SVRDEBUG("mol_mkdir reply is=%d\n",fd);
