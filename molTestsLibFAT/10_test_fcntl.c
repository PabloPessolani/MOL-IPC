...
//****************************** MODIFICADO PAP :  END  ****************************
	...
	//fd = mol_open(argv[1], O_RDWR | O_NONBLOCK);	//Para test de opendir
	fd = mol_open(argv[1], O_RDONLY | O_NONBLOCK);
	...
	retFcntl = mol_fcntl(fd, F_GETFD, 0);	
	...
	TESTDEBUG("MOL fcntl results are=%d\n", retFcntl);
