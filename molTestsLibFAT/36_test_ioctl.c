...
//****************************** MODIFICADO PAP :  START ****************************
	if ( argc != 4)	{
		fprintf(stderr, "Usage: %s <MolFilename> <request> <data>\n", argv[0] );
		exit(1);
	}
	...
	request = atoi(argv[2]);
	SVRDEBUG("(%d)Starting %s in dcid=%d endpoint=%d MolFilename=%s request=%d data=%s\n",
	         clt_lpid, argv[0], dc_ptr->dc_dcid, clt_ep, argv[1], request,argv[3]);
...
//****************************** MODIFICADO PAP :  END  ****************************
	...
	retioctl = mol_ioctl(fd, request, buffer);
		
	if( retioctl < 0 ) {
		fprintf(stderr,"mol_ioctl fd=%d\n",retioctl);
		exit(1);
	}
	TESTDEBUG("MOL mol_ioctl results are=%d\n", retioctl);
