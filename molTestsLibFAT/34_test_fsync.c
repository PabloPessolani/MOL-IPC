/*
	...
	TEST FOR FORCED SYNC !!!!!!!!!!!!!!!!
	dummyMolFilename is only for template parameters compliant 
...
//****************************** MODIFICADO PAP :  START ****************************
	if ( argc != 2 && argc != 3)	{
		fprintf(stderr, "Usage: %s <dummyMolFilename> [buf_size] \n", argv[0] );
		exit(1);
	}
	...
	SVRDEBUG("(%d)Starting %s in dcid=%d endpoint=%d dummyMolFilename=%s buf_size=%d\n",
