...
//****************************** MODIFICADO PAP :  END  ****************************
	...
	/*----------------------FILE_CLOSE-------------------*/
	rcode = mol_close(1);//close the STDOUT
	if( rcode != 0 ) {
	...
	printf("Test MOLDUP from STDOUT");
		
	TESTDEBUG("mol_dup reply is=%d\n", dupedFd);	

	/*----------------------FILE_READ-------------------*/
	bytes = mol_read(dupedFd, (void *) buffer, buf_size); 
	if( bytes < 0) {
		fprintf(stderr," mol_read (dupedFd) ERROR rcode=%d\n",bytes);
		exit(1);
	}
