...
_mnx_Mode_t parseMode(const char* mode)
{
	...
	// TESTDEBUG("MODE OWN=%c\n", mode[0]);
	// TESTDEBUG("MODE GRP=%c\n", mode[1]);
	// TESTDEBUG("MODE OTH=%c\n", mode[2]);
...
//****************************** MODIFICADO PAP :  START ****************************
	if ( argc != 3 && argc != 4)	{
		fprintf(stderr, "Usage: %s <MolFilename> <root> [buf_size] \n", argv[0] );
		exit(1);
	}
...
//****************************** MODIFICADO PAP :  END  ****************************	
	...
	if (becomeRoot)
		retChown = mol_chown(argv[1], USER_ID, GROUP_ID);
	else
		retChown = mol_chown(argv[1], USER_ID_TO_CHANGE, GROUP_ID_TO_CHANGE);
