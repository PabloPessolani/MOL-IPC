...
mnx_mode_t parseMode(const char* mode)
{

	mnx_mode_t nuevoModo;
...
void  main( int argc, char *argv[])
{
	...
	// Para UMASK
	char *mode;
	mnx_mode_t umaskMode;

//****************************** MODIFICADO PAP :  START ****************************
	if ( argc != 2 && argc != 3)	{
		fprintf(stderr, "Usage: %s <umaskMode> [buf_size] \n", argv[0] );
		exit(1);
	}
...
//****************************** MODIFICADO PAP :  END  ****************************
	...
	mode = argv[1];	
	umaskMode = parseMode(mode);
	TESTDEBUG("mode=%s\n", mode);
	TESTDEBUG("umaskMode=%d\n", umaskMode);
	TESTDEBUG("umaskMode=%X\n", umaskMode);
	...
	/*----------------------UMASK-------------------*/
	fd = mol_umask(umaskMode);
		
	if( fd < 0 ) {
		fprintf(stderr,"mol_umask fd=%d\n",fd);
		exit(1);
	}
	TESTDEBUG("MOL mol_umask reply is=%d\n",fd);	
