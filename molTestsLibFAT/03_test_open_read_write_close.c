...
//****************************** MODIFICADO PAP :  END  ****************************
	...
	SVRDEBUG("CLIENT buffer address=%X point=%X\n", buffer, *buffer);

	/*----------------------FILE_OPEN-------------------*/
	fd = mol_open(argv[1], O_RDWR);

	if ( fd < 0 ) {
		fprintf(stderr, "mol_open fd=%d\n", fd);
		exit(1);
	}
	SVRDEBUG("MOL fd is=%d\n", fd);
	...
	SVRDEBUG("bytes read count is=%d\n", bytes);
	SVRDEBUG("Buffer read is=%s\n", buffer);

	/*----------------------FILE_WRITE-------------------*/
	strcpy(buffer, "TESTING NEW WRITING!!!!\n");
