...
//****************************** MODIFICADO PAP :  START ****************************
	if ( argc != 3 && argc != 4)	{
		fprintf(stderr, "Usage: %s <MOLFilename> <access> [buf_size] \n", argv[0] );
		exit(1);
	}
...
//****************************** MODIFICADO PAP :  END  ***************************
	...
	accessToTest = atoi(argv[2]);

	// #define R_OK    4/* test for read permission */
	// #define W_OK    2/* test for write permission */
	// #define X_OK    1/* test for execute (search) permission */
	// #define F_OK    0/* test for presence of file */
	TESTDEBUG("CLIENT accessToTest=%d,  accessToTest(Hex)=%X\n", accessToTest, accessToTest);
	...
	/*----------------------ACCESS-------------------*/
	fd = mol_access(argv[1], accessToTest);	//Para test de access
		
	if( fd < 0 ) {
		fprintf(stderr,"mol_access fd=%d\n",fd);
		exit(1);
	}
	TESTDEBUG("mol_access reply is=%d\n",fd);
