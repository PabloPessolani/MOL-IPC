...
//****************************** MODIFICADO PAP :  START ****************************
	if ( argc != 3 && argc != 4)	{
		fprintf(stderr, "Usage: %s <MOLFilename> <bytesToTruncate> [buf_size] \n", argv[0] );
		exit(1);
	}
...
//****************************** MODIFICADO PAP :  END  ***************************
	...
	bytesToTruncate = atoi(argv[2]);

	TESTDEBUG("CLIENT bytesToTruncate=%d\n", bytesToTruncate);
	...
	/*----------------------TRUNCATE-------------------*/
	fd = mol_truncate(argv[1], bytesToTruncate);	//Para test de truncate
		
	if( fd < 0 ) {
		fprintf(stderr,"mol_truncate fd=%d\n",fd);
		exit(1);
	}
	TESTDEBUG("mol_truncate reply is=%d\n",fd);
