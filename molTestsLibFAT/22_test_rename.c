...
//****************************** MODIFICADO PAP :  START ****************************
	if ( argc != 3 && argc != 4)	{
		fprintf(stderr, "Usage: %s <oldMOLFilename> <newMOLFilename> [buf_size] \n", argv[0] );
		exit(1);
	}
	...
	SVRDEBUG("(%d) Starting %s in dcid=%d endpoint=%d oldMOLFilename=%s newMOLFilename=%s buf_size=%d\n",
	         clt_lpid, argv[0], dc_ptr->dc_dcid, clt_ep, argv[1], argv[2], buf_size);
...
//****************************** MODIFICADO PAP :  END  ****************************	
	...
	/*----------------------RENAME-------------------*/
	fd = mol_rename(argv[1], argv[2]);	//Para test de rename
		
	if( fd < 0 ) {
		fprintf(stderr,"mol_rename fd=%d\n",fd);
		exit(1);
	}
	TESTDEBUG("mol_rename reply is=%d\n",fd);
