...
int MOL_Bind()
{
	...
	INFODEBUG("BIND CLIENT vmid=%d fuse_pid=%d fuse_tid=%d clt_nr=%d fuse_ep=%d\n",
		vmid,
		fuse_pid,
		fuse_tid,
		clt_nr,
...
int MOL_UnBind()
{
	...
	clt_nr = 2;
	mnx_unbind(vmid,clt_nr);

	INFODEBUG("UNBIND CLIENT vmid=%d fuse_pid=%d fuse_tid=%d clt_nr=%d fuse_ep=%d\n",
		vmid,
		getpid(),
		fuse_tid,
		clt_nr,
