...
char* getAllButFirst(const char *input, char* fixed)
{
...
static int mfs_getattr(const char *path, struct stat *stbuf)
{
	...
	MOL_Bind();
	/*---------------------- BUFFER ALIGNMENT-------------------*/
	// rcode = posix_memalign( (void**) &st, getpagesize(), MAXCOPYBUF); //OJO VER ESTO
	// if( rcode ) {
	// 	ERROR_EXIT(rcode);
	// 	exit(1);
	// }
	// TESTDEBUG("CLIENT buffer address=%X point=%X\n", stbuf, *stbuf);
	res = MOL_stat(fixedPath, stbuf);
	TESTDEBUG("st_size=%d \n", stbuf->st_size);
	TESTDEBUG("st_nlink=%d \n", stbuf->st_nlink);
	TESTDEBUG("st_dev=%d \n", stbuf->st_dev);
	TESTDEBUG("st_ino=%d \n", stbuf->st_ino);
	TESTDEBUG("st_mode=%d \n", stbuf->st_mode);
	...
	// printStats(path, st);
    if (res != 0)
		res = mfs_error("mfs_getattr -> MOL_stat");	
