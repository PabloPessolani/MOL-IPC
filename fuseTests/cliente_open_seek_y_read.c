...
void  main ( int argc, char *argv[] )
{
	...
	int bytesCount = 0;
	int lseekOffset = 0;
	int lseekWhence = SEEK_SET;
	int fileDescriptor = 0;

	printf ("Cant Argumentos %d\n", argc);
	if ( argc != 9)
	{
		printf( "Usage: %s <vmid> <clt_nr> <svr_nr> <path> <bytesCount> <lseekOffset> <lseekWhence> <tipoMensaje M1=1 o M3=3> \n", argv[0] );
		exit(1);
	}
	...
	lseekOffset = atoi(argv[6]);
	lseekWhence = atoi(argv[7]);

	tipoMensaje = atoi(argv[8]);
	...
	// m.m2_l1 = offset;
  	// m.m2_i2 = whence;
	printf("CLIENT pause before SENDREC (LSEEK)\n");
	sleep(2);
...
// #define      SEEK_END        2
	...
	// //armado del mensaje LSEEK
	m.m_type = MOLLSEEK;
    m.m2_i1 = fileDescriptor; //fileDescriptor obtenido
    m.m2_l1 = lseekOffset; // offset a desplazar
    m.m2_i2 = lseekWhence; // whence SEEK_SET por ejemplo

    printf("SENDREC msg LSEEK: m_type=%d, m2_i1=%d, m2_l1=%d, m2_i2=%d\n",
    	m.m_type,
    	m.m2_i1,
    	m.m2_l1,
    	m.m2_i2);
    ...
    printf("REPLY LSEEK msg: m_source=%d, m_type=%d, m1_i1=%d, m1_i2=%d, m1_p1=%p\n",
    	m.m_source,
    	m.m_type,
    	m.m1_i1,
    	m.m1_i2,
